...
Many interesting phenomena in quantum systems have a length scale associated with the interaction.
Perhaps one of the most fundamental are the effects of either Bose-Einstein or Fermi-Dirac statistics leading to bunching or antibunching of particles \cite{Naraschewski1999.PRA.59.4595}.
...
As a result, tools that provide information regarding the spatial correlation in these systems provide invaluable insights. 
One of the most ingenious ways of probing spatial correlations due to quantum statistics was demonstrated with metastable helium atoms where the two-body and three-body pair correlation functions of quantum degenerate Bose and Fermi gases were reconstructed by dropping the atoms on to a time-resolved position-sensitive MCP detector \cite{Schellekens2005.Science.310.648, Jeltes2007.Nature.445.402, Hodgman2011.Science.331.1046}.
Although powerful, this method is not \textit{in situ} and destructive, needing to be at equilibrium before the drop in order for a reconstruction to make sense, and thus hindering the possibility of mapping the temporal development of the spatial correlations. 

Alternative, less destructive, \textit{in situ} have also been developed and employed but these are generally restricted by the dimensionality of a system and/or the length scales they are able to probe.
For the shortest length scales (i.e., $R = 0$ contact interactions), inelastic loss processes require multiple atoms to be at the same place and can be used to measure the probability of finding multiple atoms at the same\cite{Burt1997.PRL.79.337}. 
Length scales of $R \sim R_{vdW} \lesssim \SI{10}{\nm}$, photoassociative spectroscopy can be used since these effects take place on the length scale of the van der Waals interaction ($R_{vdW}$) \cite{Jones2006.RMP.78.483, Jones2006.RMP.78.1041}.
If the interaction occurs on a two (or one) dimensional lattice structure, quantum gas microscopes (QGM) have the capability of directly detecting the presence or absence of atoms on a particular lattice site and, therefore, are able to directly measure spatial correlations \cite{Bakr2010.Science.329.547, Omran2015.PRL.115.263001, Mazurenko2017.Nature.545.462}.
Since QGMs require an optical lattice, the length scales they are able to probe are restricted by the optical wavelengths used to form the lattice with typical $R \gtrsim \SI{266}{\nm}$.
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		**** PLACEHOLDER FIGURE ****.
		Various tools for probing \textit{in situ} spatial correlations.}
\end{figure}

As seen in \cref{fig:correlation_tools}, there is a gap for length scales $\SI{10}{\nm} \lesssim R \lesssim \SI{200}{\nm}$ which can be filled by ULRRMs. 
Analogous to how photoassociation can be used to probe correlations on the order of $R_{vdW}$, the formation rate of ULRRMs can also be used to measure spatial correlations. 
The advantage of ULRRMs is that the ``length scale'' being probed depends on the size of the position of the Rydberg electron's outer classical turning point, i.e., the ``length scale'' can be tuned by changing $n$. 
This tunability gives ULRRMs a huge advantage and can fill the gap between the (essentially) $R = 0$ and $R \gtrsim \SI{266}{\nm}$ length scales as seen in \cref{fig:correlation_tools}.
...
In particular, the most deeply-bound vibrational ULLRM state is well-localized and is related to the classical electron turning point of the parent Rydberg atom (see \cref{fig:vary_n-nu0}). 
\begin{figure}[!htbp]
	\centering
