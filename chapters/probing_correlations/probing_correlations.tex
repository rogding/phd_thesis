...
\Cref{fig:vibrational_pol_unpol_ratio} also shows the expected $g^{\pqty*{2}}\pqty*{R}$ for fermions at $T = \SI{850 +- 100}{\nano\kelvin}$ with $R_{n}^{\nu=0} \simeq 1.87 \pqty*{n-\delta}^{2} \si{\bohr}$\footnote{This scaling is obtained from fitting the theoretically calculated ULRRM wave functions.}.
...
Some insights can be gained by examining the radial vibrational wave functions for the ULRRM states (e.g., see \cref{fig:n34_rydberg_molecule_wave_functions_and_spectra}).
...
Notice that the higher vibrational states have a node near in the outermost well meaning the contribution to $\mathcal{F}_{n}^{\nu = 0}$ near $R_{n}^{\nu=0}$ is reduced. 
The $\nu = 1$ state can be almost be thought of as being located in the second outermost well. *** FINISH ***
