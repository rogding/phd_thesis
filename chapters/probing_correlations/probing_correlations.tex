...
In order to eliminate any dependency of $\mathcal{S}_{n}^{\nu}$ on the detection efficiency, the ratios of the measurements were compared within polarized-polarized and unpolarized-unpolarized samples.
The signals shown in \cite{fig:excited_dimer_ratios} are
\begin{equation}
	\frac{\mathcal{S}_{n,x}^{\nu=1}}{\mathcal{S}_{n,x}^{\nu=0}}
\end{equation}
where $x$ represented spin-polarized ($\mathcal{S}_{n,\text{pol}}^{\nu=1}$) or unpolarized ($\mathcal{S}_{n,\text{unpol}}^{\nu=1}$).
\begin{figure}[!htbp]
	\centering
...
\begin{equation}
	\frac{\mathcal{S}_{n}^{\nu=1}}{\mathcal{S}_{n}^{\nu=0}}	= (EQUATION WHERE FACTORS GO AWAY)
\end{equation}
