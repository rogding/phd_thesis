...
\Cref{fig:ulrrm_wave_functions_vs_n} shows the {$\nu=0$, $1$, and $2$} wave functions to provide an illustrative example of how they change with $n$.
\begin{figure}[!htbp]
	\centering
...
\end{figure}
It is clear from this figure that, \textit{a priori}, the $\nu = 1$ and $\nu = 2$ should not be expected to follow the simple scaling with $n$ that the $\nu = 0$ state does due to the additional structure. 
...
Notice that the higher vibrational states ($\nu = 1$ and $2$) have a node near the outermost well meaning the contribution to $\mathcal{F}_{n}^{\nu = 0}$ near $R_{n}^{\nu=0}$ is reduced. 
