...
In a three-dimensional lattice, Bragg scattering was employed to detect antiferromagnetic correlations in an ultracold Fermi gas \cite{Hart2015.Nature.519.211, Duarte2014.PhD} but again, this techniques is only suitable for very specific circumstances. 
Since these require an optical lattice, their accessible length scales are also restricted by the wavelength of light used to form the lattice ($R \gtrsim \SI{266}{\nm}$).
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5.25in, height=\textheight]{probing_correlations/ulrrm_photoassociation/g2_cartoon_wfn.pdf}
	\caption[]{
...
During excitation, the ODT was turned off to eliminate AC Stark shifts and then turned back on to recapture the remaining ground-state atoms after any Rydberg atoms produced are field ionized and detected with the MCP.
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5.25in, height=\textheight]{probing_correlations/spin-polarization/checking_spin-polarization/n33-check_spin-polarization.pdf}
	\caption[]{
		...
		The small features on the extreme left and right ($m_F = \pm {11}/{2}$) are likely due to imperfect laser polarization.}
\end{figure}
...
Fits to the scaled spectra in \cref{fig:processing_ULRRM_signal} are shown in \cref{fig:n34_fitted_dimers}.
\begin{figure}[!htbp]
	\centering
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5.25in, height=\textheight]{probing_correlations/detection_efficiency_vs_B/B-field-set_points/detection_efficiency_vs_B.pdf}
	\caption[]{
...
Since the $\nSLJ{5s37s}{3}{S}{1}$ state has $g_J = 2$, this corresponds to $B_{\text{pol}} = \SI{1.1882+-0.0020}{\gauss}$, which is in reasonable agreement with previous calibration measurements obtained by performing spectroscopy on the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{3}{P}{1}$ transition.
...
Comparison of spin-polarized to unpolarized signals is more difficult as the measurement requires more accurate knowledge of, and is sensitive to, experimental factors which are not common between the conditions under which data is obtained for the spin-polarized and unpolarized samples. 
