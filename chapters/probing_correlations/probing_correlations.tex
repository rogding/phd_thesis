...
The spectra of spin-polarized and unpolarized $\nSLJf{5s34s}{3}{S}{1}{{11}/{2}}$ Rydberg molecules is shown in \cref{fig:processing_ULRRM_signal} as an example.
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=\textwidth, height=\textheight]{probing_correlations/excited_dimer_signals/excited_dimer-n34-raw_signal.png}
	\caption[]{
		\label{fig:processing_ULRRM_signal}
		**** PLACEHOLDER FIGURE ****.
		**** Show ``raw'' and ``processed'' signals ready for fitting. ****}
\end{figure}

In the experiment, the photoexcitation signal of the ULRRM dimers can be modeled as
\begin{equation}
	\mathcal{S}_{n}^{\nu}\pqty{f}	=	\alpha\pqty{B} I_{\SI{689}{\nm}} I_{\SI{320}{\nm}} \mathcal{N}_2 \beta_{n} \mathcal{C} \ev{\abs{\mathcal{F}_{n}^{\nu}}^2}_{E} L\pqty{f}
\end{equation}
where $\alpha\pqty*{B}$ is the bias-field dependent detection efficiency, $I_{\SI{689}{\nm}}$ and $I_{\SI{320}{\nm}}$ are the intensities of the two-photon excitation lasers, $\mathcal{N}_2 \equiv \int \dd[3]{r} \rho\pqty*{R}^2$ is the volume integral of the square of the density distribution, $\mathcal{C}$ is a Clebsch-Gordan factor, and $L\pqty{f}$ is a normalized lineshape factor.
...
This difference in detection efficiency is represented by the $\alpha\pqty*{B}$ term in \cref{eq:ulrrm_signal}.
To determine the $\alpha\pqty*{B}$ factor, a separate study was conducted with \Sr{84} due to its simpler electronic structure. 
...
This seems unlikely as ..... **** FINISH LATER ****
