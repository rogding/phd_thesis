...
\end{figure}
Starting from a pair of colliding atoms, their scattering wave function can be written as $\ket*{\chi^{E}_{0}}$ where, well-outside the interatomic potential, they can be described by free-particle wave functions wave vector $k = \sqrt{{2 \mu E}/{\hbar^2}}$ separated by $2R$ in the center-of-mass frame.
...
\end{figure}
With the exception of the data taken at $n = 31$, both the $\nu = 0$ and $\nu = 1$ ratios exhibit a pronounced decrease in their values at smaller $n$ (i.e., smaller interparticle separations $R$) which is evidence of antibunching. 
\Cref{fig:vibrational_pol_unpol_ratio} also shows the expected $g^{\pqty*{2}}\pqty*{R}$ for fermions at $T = \SI{850 +- 100}{\nano\kelvin}$ with $R_{n}^{\nu=0} \simeq 1.87 \pqty*{n-\delta}^{2} \si{\bohr}$\footnote{This scaling is obtained from fitting the theoretically calculated ULRRM radial vibrational wave functions.}.
...
Some insights can be gained by examining the shapes of the ULLRM radial vibrational wave functions.
\Cref{fig:ulrrm_wave_functions_vs_n} shows the {$\nu=0$, $1$, and $2$} wave functions for a range of $n$ to provide an illustrative example of how they change with $n$.
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5.25in, height=\textheight]{probing_correlations/ulrrm_wave_functions/ulrrm_wave_functions_vs_n.pdf}
	\caption[]{
...
Notice that the higher vibrational states ($\nu = 1$ and $2$) have a node near in the outermost well meaning the contribution to $\mathcal{F}_{n}^{\nu = 0}$ near $R_{n}^{\nu=0}$ is reduced. 
Although the $\nu = 1$ state is delocalized, it can almost be considered as being (somewhat) localized in the second outermost potential well(s) which should contribute the most to $\mathcal{F}_{n}^{\nu = 1}$.
This ``intuition'' likely cannot be easily applied to the $\nu = 2$ ULRRM state as it is far more delocalized.
