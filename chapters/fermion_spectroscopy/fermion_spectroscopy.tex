...
In these samples, quantum statistics are expected to affect the excitation rate of ULRRMs where bosonic and fermionic samples are expected to exhibit enhanced and suppressed ULRRM excitation rates, respectively, when compared to a classical gas. 
...
The lack of nuclear spin leads to relatively simple excitation spectrum that is relatively well understood \cite{Esherick1977.PRA.15.1920, Beigang1982.OC.42.19, Beigang1982.PS.26.183, Kunze1993.ZPD.27.111, Dai1995.JQSRT.54.1019, Dai1995.PRA.52.4416, Philip2007.OC.279.141, Jackson2018.PhD, Couturier2019.PRA.99.022503}, making it easy to locate \Sr{84} Rydberg states upon appropriate rescaling of the Rydberg constant and using the appropriate ionization limit. 
...
In order to better understand the \Sr{87} Rydberg series with the goal of producing ULRRMs in a fermionic gas, we mapped the triplet Rydberg series by directly exciting $\nSLJ{5sns}{3}{S}{1}$ and $\nSLJ{5snd}{3}{D}{1,2,3}$ hyperfine states.
To our knowledge, this is the first measurement of the \Sr{87} Rydberg states by direct two-photon excitation through the $\nSLJ{5s5p}{3}{P}{1}$ intermediate state. 
...
\begin{equation}
	\ev*{\hat{V}_{\text{hf}}}	=	\frac{1}{2} A_{\text{hf}} \ev*{\hat{\va{F}}^2 - \hat{\va{J}}^2 - \hat{\va{I}}^2}
...
The experimental work was undertaken at Rice University with theory collaborates at the Vienna University of Technology. 
...
As mentioned in the paper, various energy splittings decrease with increasing $n$ whereas the hyperfine interaction remains independent of $n$.
...
From \cref{fig:n-scaling}, it's possible to estimate where hyperfine state mixing will occur by finding when $\abs*{a_{5s}}$ becomes comparable to a particular energy splitting.
...
\begin{equation}
	\hat{V}_\text{hf}	=	A \hat{\va{I}} \vdot \hat{\va{J}}
						=	\sum_{i} A_{i} \hat{\va{I}} \vdot \hat{\va{j}}_{i}
						=	\sum_{i} A_{i} \hat{\va{I}} \vdot \pqty{\hat{\va{l}}_{i} + \hat{\va{s}}_{i}}
\end{equation}
...
\begin{equation}
	\ev*{\hat{V}_\text{hf}}_{i}	=	A_{i} \ev*{\hat{\va{I}} \vdot \hat{\va{j}}_{i}}
...
Since $\ev*{\hat{V}_\text{hf}}_{i} \sim {1}/{n^{3}}$ \cite{Gudde1993.PRA.47.4725, Sobelman.Atomic, Bransden.Atoms}, the interaction of the ``outer'' electron with the nuclear spin quickly decreases with $n$ meaning $\ev*{\hat{V}_\text{hf}}_{\text{outer}} = A_{\text{outer}} \ev*{\hat{\va{I}} \vdot \hat{\va{j}}_{\text{outer}}} \approx 0$.
Therefore, the hyperfine interaction can be approximated as
\begin{equation}
	\ev*{\hat{V}_\text{hf}}	\simeq	\ev*{\hat{V}_\text{hf}}_{\text{inner}}
			=	A_{\text{inner}} \ev*{\hat{\va{I}} \vdot \hat{\va{s}}_{\text{inner}}}	
	\implies	\hat{V}_{\text{hf}}
		\simeq	A_{\text{inner}} \hat{\va{I}} \vdot \hat{\va{s}}_{\text{inner}}
\end{equation}
...
Following from a derivation presented by Shuhei Yoshida, it should be noted that $\hat{V}_{\text{hf}}$ can be expanded as
\begin{equation}
	\label{eq:Vhf_expanded}
	\hat{V}_{\text{hf}}	=	A_{\text{inner}} \hat{\va{I}} \vdot \hat{\va{s}}_{\text{inner}}
						=	\frac{1}{2}	A_{\text{inner}} \pqty{\hat{s}_{\text{inner},+} \hat{I}_{-} + \hat{s}_{\text{inner},-} \hat{I}_{+} + 2 \hat{s}_{\text{inner},z} \hat{I}_{z}}
\end{equation}
where $\hat{J}_{\pm} = \hat{J}_{x} \pm \iu \hat{J}_{y}$ and $\hat{J}_{z}$ are the ladder operators for a spin $\va{J}$.
This will be helpful when computing the matrix elements of $\hat{V}_{\text{hf}}$.
Note that $\hat{V}_{\text{hf}}$ leaves the total angular momentum $m_F$ unchanged meaning it only couples states with the same $m_F$.
...
It can be shown that $\hat{V}_{\text{hf}}$ has the following effect on the basis states (the case of $m_F = F$ was evaluated for simplicity)
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{3}{S}{1}{I-1}}	=	-\frac{1}{2} A_{5s} \pqty{I + 1} \ket{\nSLJf{5sns}{3}{S}{1}{I-1}}
\end{equation}
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{1}{S}{0}{I}}	=	\frac{1}{2} A_{5s} \sqrt{I \pqty{I + 1}} \hat{V}_{\text{hf}} \ket{\nSLJf{5sns}{3}{S}{1}{{9}/{2}}}
\end{equation}
\begin{equation}
...
\begin{equation}
	...
	\hat{V}_{\text{hf}}	=	\frac{1}{2} A_{5s}
							...
							\end{pmatrix}
\end{equation}
From \cref{eq:Vhf_s_states}, it is clear that the $\ket*{\nSLJf{5sns}{3}{S}{1}{I-1}}$ and the $\ket*{\nSLJf{5sns}{3}{S}{1}{I+1}}$ states are not mixed, exhibiting constant shift.
In contrast, the submatrix for the two $F = I$ states has off-diagonal elements meaning those states are coupled by $\hat{V}_{\text{hf}}$.
...
Incorporating the $2 I + 1 = 10$ nuclear spin states in \Sr{87} results in a total of $\num{200}$ states which can be coupled by $\hat{V}_{\text{hf}}$. 
...
A longer cell and a more sensitive detection method may be necessary since most reported {\tsup{127}I\tsub{2}} spectroscopy studies in this wavelength range used cells longer than $\SI{30}{\cm}$ in addition implementing some sort of modulation spectroscopy to improve the signal-to-noise ratio (e.g., \cite{Sansonetti1997.JOSAB.14.1913, Huang2018.AO.57.2102, Onishchenko2019.PRA.99.052503}).
...
Ya-Ting Chang, Danyel Cavazos, and Dr. Randy Hulet were kind enough to let us run a fiber\footnote{Also a Thorlabs SM600 SM fiber.} between the Killian and Hulet laboratories in order to measure the wavelengths of their $\SI{671}{\nm}$ and $\SI{646}{\nm}$ lasers used for laser cooling \Li{6}.
...
In addition to keeping the wavemeter in-place, the wavelength calibration measurements were performed within about $\SI{2}{\hour}$ to reduce sensitivity to environmental changes.
\begin{table}[!htbp]
	\caption{
