...
In these samples, quantum statistics is expected to affect the excitation rate of ULRRMs where bosonic (fermionic) gases are expected to exhibit enhanced (suppressed) excitation rates whereas no change is expected in a classical gas. 
Therefore, in order to produce ULRRMs, it is necessary to locate and identify the parent Rydberg states. 

An ultracold bosonic gas is represented by \Sr{84} due to the ease of evaporation but, in principle, any of the bosonic isotopes can be used\footnote{The ``ideal'' bosonic gas would better represented by \Sr{88} as $a_{s} \approx 0$ but this isotope difficult to evaporate.}.
...
It is this hyperfine interaction significantly complicates the Rydberg excitation spectrum.
...
R.D., S.Y. and, F.B.D. wrote the paper. 
...
\Cref{fig:results-qd-1S0,fig:results-qd-3S1,fig:results-qd-1D2,fig:results-qd-3D1,fig:results-qd-3D2,fig:results-qd-3D3} expand on the data presented in \cref{fig:Rydberg-Ritz-S,fig:Rydberg-Ritz-D}.
These present a comparison between quantum defects obtained from measurements in \Sr{88} with those from all the measured hyperfine states in \Sr{87} presented above (not just the $\nSLJf{5sns}{3}{S}{1}{{11}/{2}}$ state). 
Quantum defects from the \Sr{87} data were obtained by first subtracting the calculated hyperfine energy shift and then applying \cref{eq:Rydberg-Ritz} with the appropriate Rydberg constant and ionization limit for \Sr{87}. **** ADD VALUES USED IN PLOTS ****
...
***** NEED TO FIX 5snd 3D2.pgf FIGURE *****
\begin{figure}[!htbp]
	\centering
...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla nisl magna, vehicula id ipsum ac, rutrum viverra nibh. Vestibulum lacus dolor, placerat non erat id, lacinia dignissim purus. In eget sagittis mi. Curabitur feugiat, sapien at elementum sagittis, felis ante blandit orci, ac scelerisque sapien nisi ut arcu. Sed consequat a nibh at vestibulum. Integer sem lacus, venenatis ac efficitur quis, tempus a ante. Praesent mollis velit nec lectus pulvinar convallis. Integer aliquet placerat justo quis ornare. Vestibulum mollis volutpat ligula ut posuere. Vestibulum ornare fermentum eros sed placerat. Sed lacinia ligula elit, vitae interdum massa dignissim quis. Vestibulum nec rutrum mauris. Phasellus ac ornare odio. Pellentesque eget pellentesque nunc.

Proin fringilla purus nec fermentum eleifend. Maecenas a mattis urna, non consequat lacus. Cras vitae tempor tortor. Sed laoreet lacinia tempus. Phasellus a varius felis, sed cursus felis. Ut dictum, nunc eget imperdiet pharetra, metus nunc condimentum est, eu semper nisi ipsum sed libero. Maecenas vel odio eget risus commodo aliquam quis eget ipsum. In suscipit augue eget sapien ultrices convallis. Quisque lobortis faucibus risus, sit amet aliquet est blandit vel. Aliquam id nulla tellus. Aliquam vulputate lacinia semper. Fusce eu tellus malesuada, congue tortor non, fermentum leo. Praesent nulla ante, sagittis quis mauris sed, lacinia faucibus augue. Nam tincidunt lectus non lacus porttitor, id tempus justo tempus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Phasellus efficitur mollis mollis.
