...
\begin{figure}[h]
	...
	\caption{
		...
		(Blue) Isotopic ionization limits ($E^{\pqty{0}}_{\text{ion}}$) vs. mass.
		A linear fit (black line) is used to mass-scale $E^{\pqty{0}}_{\text{ion}}$ of the bosonic isotopes to extract the $E^{\pqty{0}}_{\text{ion}}$ for an assumed $I=0$ \Sr{87} atom (red).
		Isotopic masses from \cite{NIST_EDI} and $E^{\pqty{0}}_{\text{ion}}$ from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}.}
\end{figure}
...
The source of this discrepancy turns out to be the hyperfine interaction in the ground state of the \Srion{87}{+} which causes the $\nSLJ{5s}{2}{S}{{1}/{2}}$ to split in to $F=4$ and $F=5$ components. 
Since the \Srion{87}{+} like an alkali atom with a single valence electron outside a closed shell, the (magnetic dipole) hyperfine interaction can be written as (see, e.g., \cite{Steck.QuantumAtomOptics})
\begin{equation}
	\hat{V}_{\text{hfs}}	=	A_{\text{hfs}} \hat{\va{I}} \vdot \hat{\va{J}}
							=	A_{\text{hfs}} \hat{\va{I}} \vdot \hat{\va{S}}
\end{equation}
where $\va{J} = \va{L} + \va{S}$ and $A_{\text{hfs}}$ is the strength of the interaction.
...
\end{equation}
For the $\nSLJ{5s}{2}{S}{1/2}$ ground state of \Srion{87}{+}, $S=s={1}/{2}$ and $I={9}/{2}$ meaning $\ev{V_{\text{hfs}}} = \bqty{F \pqty{F + 1} - {51}/{2}}$ for $F=4, 5$. 

\citeauthor{Sunaoshi1993.HI.78.241} determined the magnetic dipole hyperfine coefficient to be $A_{\text{hfs}} = \SI{-1000473.673(11)}{\kHz}$ from a measurement of the ground state splitting\footnote{The hyperfine splitting of the $\nSLJ{5s}{2}{S}{1/2}$ ground state of \Srion{87}{+} in an ion trap was found to be $E_{F=4} - E_{F=5} = \SI{5002368.363(57)}{\kHz}$ \cite{Sunaoshi1993.HI.78.241}.} of an \Srion{87}{+} in an ion trap.
Therefore the $F=4$ and $F=5$ states are shifted from the $\nSLJ{5s}{2}{S}{1/2}$ state by
\begin{equation}
	\ev{V_{\text{hfs}}}	=	\begin{cases}
								-\frac{11}{4} A_{\text{hfs}} = \SI{2.751302601+-0.000000030}{\GHz}	& F=4	\\
								\frac{9}{4} A_{\text{hfs}} = \SI{-2.251065764+-0.000000025}{\GHz}	& F=5
							\end{cases}
\end{equation}
Notice that $\ev{V_{\text{hfs}}}_{F=4}$ nearly matches the $\SI{2.74}{\GHz}$ discrepancy between the reported ionization limit of \Sr{87} and the ionization limit determined by mass-scaling. 
This is reasonable since most of the earlier spectroscopic work was primarily to the singlet Rydberg states which converges to the upper $F = 4$ \cite{Beigang1982.PRA.25.1496} state. 
In the analysis below, we used \SI{45932.1943+-0.0010}{\per\cm} for the ionization limit which was calculated by subtracting $\Delta E_{F=4}$ from the reported ionization limit of \Sr{87}.
...
The theoretical approach presented below was done by Shuhei Yoshida and is presented in \cite{Ding2018.PRA.98.042505}.
...
\begin{equation}
	H_{87} = H^{0}_{88} \pqty{m_{88}, m_{87}} + V_{\text{hfs}}
\end{equation}
where $H^{0}_{88} \pqty{m_{88}, m_{87}}$ is the unperturbed Hamiltonian that yields the eigenstates and eigenenergies, i.e., spectral lines, for \Sr{88} but scaled by the reduced mass $m_{87} = \flatfrac{m_{e} M_{87}}{\pqty{m_{e} + M_{87}}}$ and $V_{\text{hfs}}$ is the hyperfine interaction, $m_{e}$ the electron mass and $M_{87}$ the mass of the \Srion{87}{+} ion.
