...
\citeauthor{Sunaoshi1993.HI.78.241} measured the $F=4,5$ hyperfine splitting in \Srion{87}{+} to be $\Delta E = \SI{5002368.363(57)}{\kHz}$, meaning $A = \SI{-1000473.673+-0.011}{\kHz}$. 
...
Assuming there are no perturbing states, ** a reasonable assumption for $n \gtrsim 12$ for the {$\nSLJ{5sns}{1}{S}{0}$-series} and $n \gtrsim 7$ for the {$\nSLJ{5sns}{3}{S}{1}$-series} \cite{Vaillant2014.JPB.47.155001} **, we start with the pure singly-excited $\ket{\nSLJF{n}{1}{S}{0}{F}}^0$ and $\ket{\nSLJF{n}{3}{S}{1}{F}}^0$ hyperfine states.
Since $\hat{V}_\text{hf}$ has rotational symmetry, it only mixes states with the same $F$.
Therefore, we work with the basis states
\begin{align}
	& \ket{\nSLJf{n}{3}{S}{1}{I+1}}	= \ket{\nSLJf{n}{3}{S}{1}{I+1}}^0																		\\
	& \ket{\nSLJf{n}{1}{S}{0}{I}}	= -\widetilde{\beta} \ket{\nSLJF{n}{3}{S}{1}{F}}^0 + \widetilde{\alpha} \ket{\nSLJF{n}{1}{S}{0}{F}}^0	\\
	& \ket{\nSLJf{n}{3}{S}{1}{I}}	= \widetilde{\alpha} \ket{\nSLJF{n}{3}{S}{1}{F}}^0 + \widetilde{\beta} \ket{\nSLJF{n}{1}{S}{0}{F}}^0	\\
	& \ket{\nSLJf{n}{3}{S}{1}{I-1}}	= \ket{\nSLJf{n}{3}{S}{1}{I-1}}^0
\end{align}
with $\widetilde{\alpha}$ and $\widetilde{\beta}$ are mixing coefficients. 
*** Need to figure out how to show that the mixing coefficients are equal to xxx maybe cite Lurio paper \cite{Lurio1962.PR.126.1758} ? ***
Using these basis states, we can compute the matrix elements of \cref{eq:V_hf}
\begin{equation}
	\mel{\nSLJf{n}{3}{S}{1}{I+1}}{\hat{V}_\text{hf}}{\nSLJf{n}{3}{S}{1}{I+1}} = \frac{1}{2} a_{5s} I \simeq \SI{-2.251}{\GHz}
\end{equation}
\begin{equation}
	\mel{\nSLJf{n}{3}{S}{1}{I-1}}{\hat{V}_\text{hf}}{\nSLJf{n}{3}{S}{1}{I-1}} = -\frac{1}{2} a_{5s} \left(I+1\right) \simeq \SI{2.751}{\GHz}
\end{equation}
\begin{align}
	\hat{V}_\text{hf} \ket{\nSLJf{n}{1}{S}{0}{I}}	&{}={}	\hat{V}_\text{hf} \left(\ket{xxxx}^0 + \ket{yyyy}^0\right)					\notag	\\
													&{}={}	\frac{1}{2} a_{ms} \sqrt{I\left(I+1\right)} \ket{\nSLJf{n}{3}{S}{1}{I}}	\\
	\hat{V}_\text{hf} \ket{\nSLJf{n}{3}{S}{1}{I}}	&{}={}	\hat{V}_\text{hf} \left(\ket{xxxx}^0 + \ket{yyyy}^0\right)					\notag	\\
													&{}={}	-\frac{1}{2} a_{ms} \left(** figure this out **\right)
\end{align}
Therefore the $\hat{V}_\text{hf}$ can be written, in matrix form, as
\begin{equation}
	\hat{V}_\text{hf}	=	\frac{1}{2} a_{ms}
							\begin{pmatrix}
								...
								0	& 0							& 0							& -\left(I+1\right)
							\end{pmatrix}
\end{equation}
** Specify basis states here!! **
Since $\hat{V}_\text{hf}$ is block-diagonal, we only need to diagonalize the $F=I$ mixed states which gives the eigenstates
\begin{align}
	\ket{??}	&{}={}	\frac{1}{\sqrt{2I+1}} \left(\sqrt{I+1}\ket{??} + \sqrt{I}\ket{??}\right)		\\
	\ket{??}	&{}={}	\frac{1}{\sqrt{2I+1}} \left(-\sqrt{I}\ket{??} + \sqrt{I+1}\ket{??}\right)
\end{align}
with eigenenergies
\begin{align}
	E_{1}	&{}={}	\frac{1}{4} \left(2\left(E_{1}+E_{3}\right) - a_{ms} + \sqrt{4\left(E_{1}-E_{3}\right)^2 + 4 a_{ms} \left(E_{1}-E_{3}\right) + a_{ms}^{2} \left(2I+1\right)^2} \right)	\\
	E_{3}	&{}={}	\frac{1}{4} \left(2\left(E_{1}+E_{3}\right) - a_{ms} - \sqrt{4\left(E_{1}-E_{3}\right)^2 + 4 a_{ms} \left(E_{1}-E_{3}\right) + a_{ms}^{2} \left(2I+1\right)^2} \right)
\end{align}
...
\end{align}
The singlet-triplet mixing is known to be strong around $n=15$ ** \cite{Esherick1977.PRA.15.1920, Beigang1981.PRL.47.326, Beigang982.PRL.48.290} - also check these**. 
...
We didn't have much luck finding narrow Doppler-free signals scanning the \SI{638}{\nm} laser through a our iodine cell\footnote{This matches similar experiences according to Jason Nguyen and Henry Luo in Randy Hulet's lab when they were looking to use the iodine cell to lock their \SI{646}{\nm} laser for a **\Li{7}** UV MOT \cite{dua2011.PRA.84.061406}.}.
