...
Focusing on two-electron systems (e.g., {\tsup{3}\text{He}}, {\tsup{9}\text{Be}}, {\tsup{25}\text{Mg}}, {\tsup{43}\text{Ca}}, \Sr{87}, {\tsup{135}\text{Ba}}, {\tsup{137}\text{Ba}}) we follow the approach adopted in \cite{Beigang1982.PRA.25.1496, Beigang1983.PRL.51.771, Beigang1988.JOSAB.5.2423}.
The Hamiltonian for two-electron systems can be written as \cite{Liao1980.OC.34.2.195}
\begin{equation}
	\hat{H} = \hat{H}_{0} + \hat{H}_{\text{fs}} + \hat{H}_{\text{hfs}}
\end{equation}
where $\hat{H}_{0}$ is the Hamiltonian without electron spin, $\hat{H}_{\text{fs}}$ accounts for the fine structure, and $\hat{H}_{\text{hfs}}$ for accounts for the hyperfine structure
...
The particular matrix elements for calculating the hyperfine shifts of the $D$ states becomes quite a bit more complicated as there are *** $2(I+1)=20$ *** interacting states.
The matrix elements of $\hat{V}_{\text{hfs}}$ can be evaluated analytically \cite{Lurio1962.PR.126.1758}\footnote{\cite{Sun1989.JPB.22.2887} seems to give the matrix elements of $\hat{V}_{\text{hfs}}$ for $F = I - 2$ to $F = I + 2$. There's mixing between $F = I - 3$ nor $F = I + 3$ states as those are only in $\SLJ{3}{D}{3}$.}.
