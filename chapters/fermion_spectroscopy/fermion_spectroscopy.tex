...
The first indication that the Rydberg levels of $\Sr{87}$ were not as simple as for the bosonic isotopes was that the reported ionization limit, $E^{\text{ion}}_{87} = \SI{45932.2861(10)}{\per\cm}$ \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}, is well above that of the bosonic isotopes (see \cref{fig:Eion-mass-scaled} and \cref{tab:ionization_limits}). 
\begin{figure}[h]
	\centering
	\includegraphics[keepaspectratio, width=4in]{fermion_spectroscopy/calculations/Eion-mass-scaled.pdf}
	\caption{
		\label{fig:Eion-mass-scaled}
		(Blue points) Reported isotopic ionization limits ($E^{\text{ion}}$) vs. isotope mass.
		A linear fit (black line) is used to extract the $E^{\text{ion}}_{87}$ for an assumed $I=0$ \Sr{87} atom from the ionization energies of the bosonic isotopes. 
		Isotopic masses were taken from \cite{NIST_EDI} and $E^{\text{ion}}$ from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}.}
\end{figure}
...
Since ground state \Srion{87}{+} has an rubidium-like structure with a single valence $s$ electron outside a filled {[Kr]} shell, the hyperfine interaction can be written as (see, e.g., \cite{Steck.QuantumAtomOptics})
\begin{equation}
...
Notice that there are no higher order contributions (e.g., electric quadrupole) since the valence electron is in an $s$ state \cite{Arimondo1977.RMP.49.31}.
Following the usual procedure of defining $\va{F} = \va{J} + \va{I}$, the energy shift is
\begin{equation}
...
Using $A_{\text{hfs}} = \SI{-1000473.673(11)}{\kHz}$\footnote{\citeauthor{Sunaoshi1993.HI.78.241} derived $A_{\text{hfs}}$ by measuring the splitting of the $\nSLJf{5s}{2}{S}{1/2}{4}$ and $\nSLJf{5s}{2}{S}{1/2}{5}$ ground state of \Srion{87}{+} in an ion trap. They measured the splitting to be $E_{F=4} - E_{F=5} = \SI{5002368.363(57)}{\kHz}$.} \cite{Sunaoshi1993.HI.78.241}, the $F=4$ and $F=5$ states are shifted from the $\nSLJ{5s}{2}{S}{1/2}$ by
\begin{equation}
...
In the analysis below, we used $E^{\text{ion}}_{87} = \SI{45932.1943+-0.0010}{\per\cm}$ for the ionization limit which was calculated by subtracting $\Delta E_{F=4}$ from the reported ionization limit of \Sr{87}.

\section{Hyperfine Rydberg States of \Sr{87}}

Accurate \textit{ab initio} calculations of the Rydberg structure of atoms more complicated than hydrogen-like atoms due to the interaction of the Rydberg electron with the ionic core electrons \cite{Gallagher1994.RydbergAtoms}.
Multichannel quantum defect theory (MQDT) is the commonly used method for analyzing the energy levels of these multielectron atoms \cite{Gallagher1994.RydbergAtoms, Aymar1996.RMP.68.1015, Robicheaux2018.PRA.97.022508}\footnote{I think \cite{Cooke1985.PRA.32.2725}, referenced and followed by \cite{Gallagher1994.RydbergAtoms}, would probably be a good paper to work through as they provide an example calculation.}. 
** Layman's explanation of MQDT? **

The theoretical approach described below was worked out by Shuhei Yoshida and is presented in \cite{Ding2018.PRA.98.042505}.
To calculate the hyperfine structure of the \Sr{87} Rydberg states, we follow the approach adopted in \cite{Beigang1982.PRA.25.1496, Beigang1983.PRL.51.771} and assume that the all isotopes of strontium share the same electron correlation and configuration interactions.
This allows us to leverage the simpler spectroscopic data of the bosonic isotopes from which we can deduce the eigenstates and eigenenergies of an assumed $I = 0$ \Sr{87} atom before incorporating isotope-specific interactions.
Since most previous studies in strontium were performed with \Sr{88}, we can write the Hamiltonian for \Sr{87} as
\begin{equation}
	H^{87}	=	H_{0}^{88} \pqty{m_{88}, m_{87}} + V_{\text{hfs}}
\end{equation}
where $H_{0}^{88} \pqty{m_{88}, m_{87}}$ is the unperturbed Hamiltonian that yields the eigenstates and eigenenergies, i.e., spectral lines, for \Sr{88} but scaled by the reduced mass $m_{87} = \flatfrac{m_{e} M_{87}}{\pqty{m_{e} + M_{87}}}$ and $V_{\text{hfs}}$ is the hyperfine interaction, $m_{e}$ the electron mass and $M_{87}$ the mass of the \Srion{87}{+} ion.
Corrections beyond the elementary isotope shift, in particular, the mass polarization correction, was estimated from earlier data for helium Rydberg states \cite{Cok1979.PRA.19.1830, Cok1981.PRA.24.3283, vanderVeldt1990.PRA.41.4099} and are expected to be $\lesssim \SI{1}{\MHz}$ and are neglected.
The terms

We describe the two-electron system with the Hamiltonian \cite{Liao1980.OC.34.2.195}
\begin{equation}
	\hat{H} = \hat{H}_{0} + \hat{H}_{\text{fs}} + \hat{H}_{\text{hfs}}
\end{equation}
where $H_{0}$ is the Hamiltonian without electron spin, $H_{\text{fs}}$ and $H_{\text{hfs}}$ for account for the fine and hyperfine structure, respectively. 
** More explicitly work out how the approximation in the equation above leads to the one below? Maybe follow \cite{Bethe.Quantum, Cok1979.PRA.19.1830, Cok1981.PRA.24.3283}**
For a singly-excited Rydberg state where the Rydberg electron is in the $\ket{n \ell}$ state while the inner electron remains in the $\ket{m s}$ state, the Hamiltonian can be approximated as \cite{Beigang1983.PRL.51.771, Beigang1988.JOSAB.5.2423}
\begin{equation}
...
The Hamiltonian for a two-electron system in an $\ket{(ms)(nl)}$ Rydberg state can be written as \cite{Beigang1983.PRL.51.771, Beigang1988.JOSAB.5.2423}
\begin{equation}
	\hat{H} = \hat{H}^{0}_{1} + \hat{H}^{0}_{2} + \frac{e^{2}}{r_{12}} + \zeta_{nl} \hat{\va{s}} \vdot \hat{\va{l}} + a_{ms} \hat{\va{s}} \vdot \hat{\va{I}}
\end{equation}
where $H^{0}_{i} = {\hat{\va{p}}_{i} \vdot \hat{\va{p}}}_{i}/{2 m_{e}} - {e^{2}}/{r_{i}}$ for the $i$th electron, ${e^{2}}/{r_{12}}$ is the Coulomb repulsion between the electrons, $\zeta_{nl}$ is the magnitude of the spin-orbit interaction (of the Rydberg electron), and $a_{ms}$ is the strength of the hyperfine interaction of the inner $s$ electron. 
For \Sr{88} with $I = 0$, we should expect $\hat{H}^{0}_{88}$ to yield the eigenstates and eigenenergies for \Sr{88}. 
...
Atoms from the Zeeman-slowed atomic beam are then captured by a blue MOT operating on the same transition which cools the atoms down to $\sim \SI{2}{\milli\kelvin}$. 
...
The hyperfine shifts for each state was calculated by Shuhei Yoshida and were subtracted from the measured term energy to obtain the quantum defect for an effective $I = 0$ \Sr{87} atom.
\begin{SingleSpace}
