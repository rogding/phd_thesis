...
\begin{figure}[h]
	...
	\caption{
		...
		(Blue) Isotopic ionization limits ($E^{\qty(0)}_{\text{ion}}$) vs. mass.
		A linear fit (black line) is used to mass-scale $E^{\qty(0)}_{\text{ion}}$ of the bosonic isotopes to extract the $E^{\qty(0)}_{\text{ion}}$ for an assumed $I=0$ \Sr{87} atom (red).
		Isotopic masses from \cite{NIST_EDI} and $E^{\qty(0)}_{\text{ion}}$ from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}.}
\end{figure}
...
\begin{equation}
	\ev{V_{\text{hfs}}}	=	\frac{A_{\text{hfs}}}{2} \ev{\va{F}^2 - \va{J}^2 - \va{I}^2}
						=	\frac{A_{\text{hfs}}}{2} \qty[F \qty(F + 1) - J \qty(J + 1) - I \qty(I + 1)]
\end{equation}
For the $\nSLJ{5s}{2}{S}{1/2}$ ground state of \Srion{87}{+}, $S=s=\flatfrac{1}{2}$ and $I=\flatfrac{9}{2}$ meaning $\ev{V_{\text{hfs}}} = \qty[F \qty(F + 1) - \flatfrac{51}{2}]$ for $F=4, 5$. 
