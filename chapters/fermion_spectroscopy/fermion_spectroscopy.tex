...
In these samples, quantum statistics are expected to affect the excitation rate of ULRRMs where bosonic and fermionic samples are expected to exhibit enhanced and suppressed ULRRM excitation rates, respectively.
The large nuclear spin of \Sr{87} also provides an approximation to a classical gas, where the states are distributed among the $2I+1 = 10$ $m_F$ sublevels of the $\nSLJf{5s^2}{1}{S}{0}{{9}/{2}}$ ground state, as comparison. 
...
An ultracold bosonic gas is represented by \Sr{84} due to the ease of evaporation but, in principle, any of the bosonic isotopes can be used\footnote{An ``ideal'' bosonic gas would be better represented with \Sr{88} as $a_{s} \approx \SI{0}{\bohr}$ but this isotope is difficult to evaporate.}.
...
Since the other bosonic isotopes also have $I = 0$, it is straightforward to locate Rydberg states in \Sr{84} upon appropriate rescaling of the Rydberg constant and using the appropriate ionization limit. 
...
\begin{figure}[htbp]
	...
	\caption{
		...
		Ionization energies from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103} and atomic masses from \cite{NIST_EDI} (see \cref{tab:ionization_limits} for additional information).}
\end{figure}
...
This value is about $\SI{2.74}{\GHz}$ below the reported ionization energy.
...
\end{figure}
The strength of hyperfine interaction can be related to the contact interaction of the inner $5s$ electron with the \Srion{87}{+} core and is taken to be $\abs*{a_{5s}} \simeq \SI{1}{\GHz}$ \cite{Sunaoshi1993.HI.78.241}. 
...
\end{equation}
for electron $i \in \Bqty{1,2}$ with the (magnetic dipole) hyperfine constant $A_{i}$.
...
This will be helpful when computing the matrix elements of $\hat{V}_{\text{hf}}$.
Note that $\hat{V}_{\text{hf}}$ leaves the total angular momentum $m_F$ unchanged, only coupling states with the same $m_F$.
...
Using \cref{eq:Vhf_expanded}, it can be shown that $\hat{V}_{\text{hf}}$ has the following effect on the basis states (the case of $m_F = F$ was evaluated for simplicity)
\begin{equation}
...
Unfortunately, we were unable to observe narrow Doppler-free signals when scanning the $\SI{638}{\nm}$ laser through a our iodine cell using a simple saturated absorption setup\footnote{This matches similar experiences reported by Jason Nguyen and Henry Luo in the Hulet lab when they were considering using an iodine cell to lock their $\SI{646}{\nm}$ laser for a \Li{6} UV MOT \cite{Duarte2011.PRA.84.061406}.}.
