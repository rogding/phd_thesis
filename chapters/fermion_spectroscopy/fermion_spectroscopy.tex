...
\end{figure}
Mass-scaling the ionization limits of the bosonic isotopes suggests that the ionization limit for an assumed $I=0$ \Sr{87} atom should be \SI{45932.1946+-0.0005}{\per\cm}, about \SI{2.74}{\GHz} below the reported ionization energy.
%This discrepancy turns out to match the hyperfine shift due to the interaction between the remaining $5s$ electron and the $I = \flatfrac{9}{2}$ core of the \Srion{87}{+}.
Since the \Srion{87}{+} like an alkali atom with a single valence electron outside a closed shell, the hyperfine interaction splits the $\nSLJ{5s}{2}{S}{\flatfrac{1}{2}}$ into $F=\num{4}$ and $F=\num{5}$ components.
As a result, the hyperfine magnetic dipole interaction can be written as (see, e.g., \cite{Steck.QuantumAtomOptics})
\begin{equation}
	V_{\text{hfs}}	=	A_{\text{hfs}} \va{I} \vdot \va{J}
					=	A_{\text{hfs}} \va{I} \vdot \va{S}
\end{equation}
where $\va{J} = \va{L} + \va{S}$.
Following the usual procedure of defining $\va{F} = \va{J} + \va{I}$, the energy shift is
\begin{equation}
	\ev*{V_{\text{hfs}}}	=	\frac{A_{\text{hfs}}}{2} \ev*{\va{F}^2 - \va{J}^2 - \va{I}^2}
						=	\frac{A_{\text{hfs}}}{2} \bqty{F \pqty{F + 1} - J \pqty{J + 1} - I \pqty{I + 1}}
\end{equation}
For the $\nSLJ{5s}{2}{S}{1/2}$ ground state of \Srion{87}{+}, $S=s=\flatfrac{1}{2}$ and $I=\flatfrac{9}{2}$ meaning $\ev{V_{\text{hfs}}} = \bqty{F \pqty{F + 1} - \flatfrac{51}{2}}$ for $F=4, 5$. 
...
In order to calculate the hyperfine structure of the \Sr{87} Rydberg states, we follow the approach adopted in \cite{Beigang1982.PRA.25.1496, Beigang1983.PRL.51.771} and assume that the all isotopes of strontium share the same electron correlation and configuration interactions.
...
Corrections beyond the elementary isotope shift, in particular, the mass polarization correction, was estimated from earlier data for helium Rydberg states \cite{Cok1979.PRA.19.1830, Cok1981.PRA.24.3283, vanderVeldt1990.PRA.41.4099} and are expected to be $\lesssim \SI{1}{\MHz}$ and are neglected \cite{Ding2018.PRA.98.042505}.
