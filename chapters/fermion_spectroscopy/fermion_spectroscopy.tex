...
The first indication that the Rydberg levels of $\Sr{87}$ were not as simple as for the bosonic isotopes was that the reported ionization limit of \SI{45932.2861(10)}{\per\cm} \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103} is well above that of all the bosonic isotopes (see \cref{tab:ionization_limits}). 
\begin{figure}[h]
	\centering
	\includesvg[keepaspectratio, width=\textwidth]{fermion_spectroscopy/calculations/Eion-mass-scaled.svg}
	\caption{
		\label{fig:Eion-mass-scaled}
		Mass-scaling the ionization limits ($I_S$) to estimate the ionization limit for \Sr{87}.
		Isotopic masses from \cite{NIST_EDI} and reported $I_S$ from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}.}
\end{figure}
Mass-scaling the ionization limits of the bosonic isotopes suggests that the ionization limit for an $I=0$ \Sr{87} atom should be \SI{45932.1946+-0.0005}{\per\cm}, about \SI{2.74}{\GHz} below the reported ionization energy.

This discrepancy turns out to match the hyperfine shift due to the interaction between the remaining $5s$ electron and the $I = \num{9/2}$ \Srion{87}{+} core.
Since the \Srion{87}{+} like a rubidium atom with a single valence electron outside a closed {[Kr]} shell, the hyperfine interaction splits the $\nSLJ{5s}{2}{S}{1/2}$ into $F=\num{4}$ and $F=\num{5}$ components.
Writing the magnetic dipole interaction as
\begin{equation}
	\ev{W} = A \ev{\hat{\bm{I}} \vdot \hat{\bm{S}}} = \frac{A}{2} \qty(F\qty(F+1) - I\qty(I+1) - S\qty(S+1))
\end{equation}
where $\bm{F} = \bm{J} + \bm{I}$ and $S=s=1/2$.
...
Therefore, the hyperfine shift of the $F=4$ state is $\Delta E_{F=4} = \flatfrac{A}{2}(\flatfrac{-11}{2}) = \SI{2.751302601+-0.000000030}{\GHz}$, roughly matching the discrepancy noted earlier meaning that the reported ionization limit is for the $\nSLJf{5s}{2}{S}{1/2}{4}$ state.
This makes sense since most of the earlier spectroscopic work was done through the singlet Rydberg states which are attached to the ** $F=4$ (CHECK!) **  state.
