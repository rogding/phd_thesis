...
As mentioned in \cref{sec:sr_rydberg_atoms}, strontium offers the possibility for exciting ULRRMs in ultracold bosonic and fermionic gases.
In these samples, quantum statistics are expected to affect the excitation rate of ULRRMs where bosonic and fermionic samples should exhibit enhanced and suppressed rates, respectively.
The large nuclear spin of \Sr{87} also provides an approximation to a classical gas, where the states are distributed among the $2I+1 = 10$ $m_F$ sublevels of the $\nSLJf{5s^2}{1}{S}{0}{{9}/{2}}$ ground state, useful as a comparison. 
In order to produce ULRRMs in ultracold strontium gases, it is first necessary to locate and identify the parent Rydberg levels. 

An ultracold bosonic gas is represented by \Sr{84} due to the ease of evaporation but, in principle, any of the bosonic isotopes can be used\footnote{An ``ideal'' bosonic gas would be better represented with \Sr{88} since $a_{s} \approx \SI{0}{\bohr}$ but this isotope is difficult to evaporate.}.
Most spectroscopic studies of strontium Rydberg systems have focused on the most abundant isotope (\Sr{88}) which has $I = 0$.
Not only is there a wealth of experimental measurements available for this isotope, the absence of nuclear spin leads to an excitation spectrum that is relatively well understood \cite{Esherick1977.PRA.15.1920, Beigang1982.OC.42.19, Beigang1982.PS.26.183, Kunze1993.ZPD.27.111, Dai1995.JQSRT.54.1019, Dai1995.PRA.52.4416, Philip2007.OC.279.141, Jackson2018.PhD, Couturier2019.PRA.99.022503}.
Since the other bosonic isotopes also have $I = 0$, it is straightforward to locate Rydberg states in \Sr{84} (or \Sr{86}) upon rescaling of the Rydberg constant and using the appropriate ionization limit.

In contrast, relatively few studies have been done on \Sr{87} which has strong hyperfine interactions due to its large $I = {9}/{2}$ nuclear spin. 
It is this hyperfine interaction that leads to mixing between states normally well-described by $LS$-coupling in the bosons, significantly complicating the Rydberg excitation spectrum.
In previous experimental work \cite{Beigang1981.ZPhys.301.93, Beigang1981.PRL.47.326, Beigang1982.PRL.48.290, Beigang1982.PRA.25.1496, Beigang1983.PRL.51.771}, excitation was performed to singlet $\nSLJ{5sns}{1}{S}{0}$ and $\nSLJ{5snd}{1}{D}{2}$ Rydberg levels with triplet states being observed due to state mixing. 
In order to better understand the \Sr{87} Rydberg states, we mapped the triplet Rydberg series by two-photon excitation to $\nSLJ{5sns}{3}{S}{1}$ and $\nSLJ{5snd}{3}{D}{1,2,3}$ hyperfine states via the $\nSLJ{5s5p}{3}{P}{1}$ intermediate state. 
To our knowledge, this is the first measurement of \Sr{87} Rydberg states by direct excitation utilizing the $\nSLJ{5s5p}{3}{P}{1}$ intermediate state. 

%**** ADD A BIT MORE OF A LEAD IN AND OVERVIEW OF CHAPTER? ****

\section{Ionization Limit of $\Sr{87}$}

The first indication that the Rydberg levels of $\Sr{87}$ were not as simple as for the bosonic isotopes was the reported ionization limit, $E_{\text{ion}}^{87} = \SI{45932.2861+-0.0010}{\per\cm}$ \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103}, being well above those of the bosons as shown in \cref{fig:Eion-mass-scaled,tab:ionization_limits}. 
\begin{figure}[htbp]
	\centering
	\includegraphics[keepaspectratio, width=5in, height=\textheight]{fermion_spectroscopy/calculations/Eion-mass-scaled.pdf}
	\caption{
		\label{fig:Eion-mass-scaled}
		Reported isotope ionization limits $E_{\text{ion}}$ vs. mass (blue points).
		...
		Ionization energies from \cite{Beigang1982.OC.42.19, Sansonetti2010.JPCRD.39.033103} and atomic masses from \cite{NIST_EDI, NIST_Comp} (see \cref{tab:sr_phys_prop,tab:ionization_limits} for additional information).}
\end{figure}
Mass-scaling the ionization energies of the bosonic isotopes suggests that the ionization energy for an assumed $I = 0$ \Sr{87} atom should be approximately $\SI{45932.1946+-0.0005}{\per\cm}$.
This value is about $\SI{2.74}{\GHz}$ below the reported ionization energy for \Sr{87}.

The source of this discrepancy is likely related to the hyperfine interaction in the ground state of \Srion{87}{+}.
Since it has a rubidium-like structure with a single $s$ valence electron outside a filled {[Kr]} core, the hyperfine interaction can be written as (see, e.g., \cite{Steck.QuantumAtomOptics, Woodgate.Elementary})
\begin{equation}
	\hat{V}_{\text{hf}}	=	A_{\text{hf}} \hat{\vb{I}} \vdot \hat{\vb{J}}
						=	A_{\text{hf}} \hat{\vb{I}} \vdot \hat{\vb{S}}
\end{equation}
where $\vb{J} = \vb{L} + \vb{S}$ and $A_{\text{hf}}$ is the strength of the magnetic dipole hyperfine interaction with $\vb{L}$, $\vb{S}$, and $\vb{J}$ referring to the ground-state of \Srion{87}{+} (singly-ionized \Sr{87}).
...
Following the usual procedure by defining $\vb{F} = \vb{J} + \vb{I}$, the energy shift can be evaluated as
\begin{equation}
	\ev*{\hat{V}_{\text{hf}}}	=	\frac{1}{2} A_{\text{hf}} \ev*{\hat{\vb{F}}^2 - \hat{\vb{J}}^2 - \hat{\vb{I}}^2}
								=	\frac{1}{2} A_{\text{hf}} \bqty*{F \pqty*{F + 1} - J \pqty*{J + 1} - I \pqty*{I + 1}}
\end{equation}
In the ground state of \Srion{87}{+}, $S=s={1}/{2}$ and $I={9}/{2}$ meaning the the $\nSLJ{5s}{2}{S}{1/2}$ is split in to {$F = 4$ and $5$} components\footnote{Note that $F$ has integer values in \Srion{87}{+} and half-integer values in \Sr{87}.}.
Using $A_{\text{hf}} = \SI{-1000473.673(11)}{\kHz}$\footnote{\citeauthor{Sunaoshi1993.HI.78.241} determined $A_{\text{hf}}$ by measuring the splitting of the $\nSLJf{5s}{2}{S}{1/2}{4}$ and $\nSLJf{5s}{2}{S}{1/2}{5}$ ground state of \Srion{87}{+} in an ion trap. They measured the splitting to be $E_{F=4} - E_{F=5} = \SI{5002368.363(57)}{\kHz}$.} \cite{Sunaoshi1993.HI.78.241}, the $F=4$ and $F=5$ states in \Srion{87}{+} are shifted by
\begin{equation}
...
This seems reasonable since most of the earlier spectroscopic work on \Sr{87} primarily focused on the singlet Rydberg states which converges to the upper $F = 4$ state of \Srion{87}{+} \cite{Beigang1982.PRA.25.1496}. 
...
The experimental work was undertaken at Rice University with theory collaborates at the Institute for Theoretical Physics of the Vienna University of Technology. 
R.D., J.D.W., and S.K.K. took the experimental data, R.D. compiled and analyzed the data, and S.Y. developed the theoretical framework for \Sr{87} and performed the calculations to extract the quantum defects from the experimental data. 
The paper was written collaboratively between R.D., S.Y., and F.B.D. 
S.Y. wrote the theoretical section, R.D. wrote the experimental section, and S.Y. and F.B.D. wrote the introduction, results, and conclusion sections.
All three discussed the introduction, results, and conclusion.
...
\label{sec:fermion_additional_materials}

The following subsections include additional material not included in the published paper. 

\subsection{$n$ Scaling of Various Splittings}

As mentioned in the paper, various energy splittings decrease with increasing $n$ whereas the hyperfine interaction remains constant.
...
\Cref{fig:n-scaling} shows the same information as  \cref{fig:deltaE} (plotted for $n$ instead of $n^{*}$) but with more splittings and was computed using the most recently reported ionization limit and quantum defects for \Sr{88} from \cite{Vaillant2012.JPB.45.135004, Couturier2019.PRA.99.022503}.
\begin{figure}[!htbp]
	\centering
	...
	\caption{
		...
		The energy scale of the hyperfine interaction in \Sr{87} is approximately constant and is taken to be $\abs*{a_{5s}} \approx \SI{1}{\GHz}$ \cite{Sunaoshi1993.HI.78.241}.}
\end{figure}
The strength of hyperfine interaction can be related to the contact interaction of the inner $5s$ electron with the \Srion{87}{+} core and is taken to be $\abs*{a_{5s}} \simeq \SI{1}{\GHz}$ \cite{Sunaoshi1993.HI.78.241} in the figure. 

From examining \cref{fig:n-scaling}, it's possible to estimate where hyperfine state mixing will occur by finding when $\abs*{a_{5s}}$ becomes comparable to a particular energy splitting.
This suggests, for example, that hyperfine mixing should occur between the $\nSLJ{5snd}{1}{D}{2}$ and the $\nSLJ{5snd}{3}{D}{2}$ states near $n = 14$ and between the $\nSLJ{5snd}{1}{D}{2}$ and the $\nSLJ{5snd}{3}{D}{3}$ states near $n = 20$.
Indeed, both interactions have been observed experimentally with large shifts reported in the spectra of \Sr{87} $\nSLJ{5snd}{1}{D}{2}$ hyperfine Rydberg states around $n = 16$ \cite{Beigang1981.PRL.47.326, Beigang1982.PRL.48.290} and $n = 19$ \cite{Beigang1982.JPB.15.L201}. 
At higher principal quantum number (around $n = 113$), $n$-mixing was also reported when the hyperfine interaction becomes comparable to the $\nSLJf{5sns}{1}{S}{0}{{9}/{2}}$ and $\nSLJf{5s\pqty{n+1}s}{3}{S}{1}{{9}/{2}}$ energy separation \cite{Beigang1983.PRL.51.771}.

\subsection{$\hat{V}_{\text{hf}}$ for Singly Excited Rydberg States}

The hyperfine interaction can be expressed as
\begin{equation}
	\hat{V}_\text{hf}	=	A \hat{\vb{I}} \vdot \hat{\vb{J}}
						=	\sum_{i} A_{i} \hat{\vb{I}} \vdot \hat{\vb{j}}_{i}
						=	\sum_{i} A_{i} \hat{\vb{I}} \vdot \pqty{\hat{\vb{l}}_{i} + \hat{\vb{s}}_{i}}
\end{equation}
for electron $i \in \Bqty{1,2}$ with the (magnetic dipole) hyperfine constant $A_{i}$ \cite{Woodgate.Elementary, Sobelman.Atomic, Bransden.Atoms}.
Therefore the contribution of each electron to the total hyperfine shift is
\begin{equation}
	\ev*{\hat{V}_\text{hf}}_{i}	=	A_{i} \ev*{\hat{\vb{I}} \vdot \hat{\vb{j}}_{i}}
								=	\frac{A_{i}}{2} \bqty*{f_{i} \pqty*{f_{i} + 1} - I \pqty*{I + 1} - j_{i} \pqty*{j_{i} + 1}}
\end{equation}
We now consider a singly-excited Rydberg state where the ``inner'' electron is in the $\ket*{m s}$ state and the ``outer'' electron is in the $\ket*{n l}$ state (with $n \gg m$).
Since $\ev*{\hat{V}_\text{hf}}_{i} \sim {1}/{n^{3}}$ \cite{Gudde1993.PRA.47.4725, Sobelman.Atomic, Bransden.Atoms}, the interaction of the ``outer'' electron with the nuclear spin quickly decreases with $n$ meaning $\ev*{\hat{V}_\text{hf}}_{\text{outer}} = A_{\text{outer}} \ev*{\hat{\vb{I}} \vdot \hat{\vb{j}}_{\text{outer}}} \approx 0$.
Therefore, the hyperfine interaction can be approximated as
\begin{equation}
	\ev*{\hat{V}_\text{hf}}	\simeq	\ev*{\hat{V}_\text{hf}}_{\text{inner}}
			=	A_{\text{inner}} \ev*{\hat{\vb{I}} \vdot \hat{\vb{s}}_{\text{inner}}}	
	\implies	\hat{V}_{\text{hf}}
		\simeq	A_{\text{inner}} \hat{\vb{I}} \vdot \hat{\vb{s}}_{\text{inner}}
\end{equation}
For \Sr{87}, the inner electron is in the $5s$ state with $A_{\text{inner}} = A_{5s} = \SI{-1000473.673(11)}{\kHz}$ obtained from the hyperfine splitting of the {$\nSLJf{5s}{2}{S}{1/2}{4}$ and $5$} ground states in \Srion{87}{+} \cite{Sunaoshi1993.HI.78.241}.

Following from the derivation presented by Shuhei Yoshida, it should be noted that $\hat{V}_{\text{hf}}$ can be expanded as
\begin{equation}
	\label{eq:Vhf_expanded}
	\hat{V}_{\text{hf}}	=	A_{\text{inner}} \hat{\vb{I}} \vdot \hat{\vb{s}}_{\text{inner}}
						=	\frac{1}{2}	A_{\text{inner}} \pqty{\hat{s}_{\text{inner},+} \hat{I}_{-} + \hat{s}_{\text{inner},-} \hat{I}_{+} + 2 \hat{s}_{\text{inner},z} \hat{I}_{z}}
\end{equation}
where $\hat{J}_{\pm} = \hat{J}_{x} \pm \iu \hat{J}_{y}$ and $\hat{J}_{z}$ are the ladder operators for a spin $\vb{J}$.
This is helpful when computing the matrix elements of $\hat{V}_{\text{hf}}$.
Note that $\hat{V}_{\text{hf}}$ leaves the total angular momentum $m_F$ unchanged, so it only couples states with the same $m_F$.

\subsection{Singly Excited $\SLJ{}{S}{}$ State Hyperfine Mixing Matrix}

%**** NEED TO DOUBLE CHECK (2019/08/09) ****

The matrix elements for hyperfine mixing of the $\nSLJ{5sns}{1}{S}{0}$ and the $\nSLJ{5sns}{3}{S}{1}$ states are not too difficult to evaluate and the derivation outlined below is based on the one presented by Shuhei Yoshida.

For the singly excited $\SLJ{}{S}{}$ states, the four basis states are $\ket*{\nSLJf{5sns}{3}{S}{1}{I-1}}$, $\ket*{\nSLJf{5sns}{1}{S}{0}{I}}$, $\ket*{\nSLJf{5sns}{3}{S}{1}{I}}$, and $\ket*{\nSLJf{5sns}{3}{S}{1}{I+1}}$.
%From \cref{eq:Vhf_expanded}, it can be seen that $\hat{V}_{\text{hf}}$ does not change $m_F$ and only couples states with the same $m_F$.
Using \cref{eq:Vhf_expanded}, it can be shown that $\hat{V}_{\text{hf}}$ has the following effect on the basis states (the case of $m_F = F$ was evaluated for simplicity since the final results should be independent of $m_F$ for a given $F$)
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{3}{S}{1}{I-1}}	=	-\frac{1}{2} A_{5s} \pqty{I + 1} \ket{\nSLJf{5sns}{3}{S}{1}{I-1}}
\end{equation}
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{1}{S}{0}{I}}	=	\frac{1}{2} A_{5s} \sqrt{I \pqty{I + 1}} \ket{\nSLJf{5sns}{3}{S}{1}{{9}/{2}}}
\end{equation}
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{3}{S}{1}{I}}	=	\frac{1}{2} A_{5s} \pqty{\sqrt{I}\pqty{I + 1} \ket{\nSLJf{5sns}{1}{S}{0}{I}} - \ket{\nSLJf{5sns}{3}{S}{1}{I}}}
\end{equation}
\begin{equation}
	\hat{V}_{\text{hf}}	\ket{\nSLJf{5sns}{3}{S}{1}{I+1}}	=	\frac{1}{2} A_{5s} I \ket{\nSLJf{5sns}{3}{S}{1}{I+1}}
\end{equation}
Therefore the total hyperfine interaction Hamiltonian can be written as
\begin{equation}
	\label{eq:Vhf_s_states}
	\hat{V}_{\text{hf}}	=	\frac{1}{2} A_{5s}
							...
							\end{pmatrix}
\end{equation}
From \cref{eq:Vhf_s_states}, it is clear that the $\ket*{\nSLJf{5sns}{3}{S}{1}{I-1}}$ and the $\ket*{\nSLJf{5sns}{3}{S}{1}{I+1}}$ states are not mixed, resulting in the states exhibiting a constant shift.
Conversely, the submatrix for the two $F = I$ states has off-diagonal elements meaning those states are coupled by $\hat{V}_{\text{hf}}$.

\subsection{Singly Excited $\SLJ{}{D}{}$ State Hyperfine Mixing Matrix}

A procedure similar to the one used to derive the $\hat{V}_{\text{hf}}$ matrix elements for singly excited $\SLJ{}{S}{}$ states can be applied to obtain $\hat{V}_{\text{hf}}$ for the singly excited $\SLJ{}{D}{}$ states but the process is quite a bit more tedious. 
Starting from the $LS$-coupled basis, there are $\num{20}$ states spread across the various $\ket*{\nSLJM{5snd}{1}{D}{2}{m_J}}$, $\ket*{\nSLJM{5snd}{3}{D}{1}{m_J}}$, $\ket*{\nSLJM{5snd}{3}{D}{2}{m_J}}$, and $\ket*{\nSLJM{5snd}{3}{D}{3}{m_J}}$ sublevels. 
Incorporating the $2 I + 1 = 10$ nuclear spin states of \Sr{87} results in a total of $\num{200}$ states which can be coupled by $\hat{V}_{\text{hf}}$. 
...
All light used in the calibration measurements was delivered to the EXFO WA-1500 wavemeter using single mode (SM) optical fibers\footnote{Thorlabs SM600 FC/PC single mode fiber.}.
We had previously used a multimode (MM) fiber but found that the wavemeter readings were inconsistent, most likely due to the speckle output pattern from the MM or to slight beam pointing deviations.
Switching to a SM fiber mitigated these issue and we were able to consistently obtain wavenumber measurements limited by the resolution of the wavemeter with a statistical uncertainty of about $\sigma_{\text{stat}} = \pm \SI{0.0005}{\per\cm} = \pm \SI{15}{MHz}$ for the wavelengths we measured. Since the $\SI{319}{\nm}$ light is generated by frequency-doubling from $\SI{638}{\nm}$, the statistical uncertainty should be doubled to about $\sigma_{\text{stat}} = \pm \SI{0.001}{\per\cm} = \pm \SI{30}{MHz}$.
...
We initially attempted to perform Doppler-free spectroscopy of molecular iodine ({\tsup{127}I\tsub{2}}) using the ${P65~\left(7–4\right)}$ line at \SI{15672.517398(25)}{\per\cm} \cite{Sansonetti1997.JOSAB.14.1913}, which is near the $\SI{638}{\nm}$ wavelength required to excite the $\nSLJ{5s40s}{3}{S}{1}$ strontium Rydberg state.
Unfortunately, we were unable to observe narrow Doppler-free signals when scanning the $\SI{638}{\nm}$ laser through a our iodine cell using a simple saturated absorption setup\footnote{This matches similar experiences reported by Dr. Jason Nguyen and Henry Luo in the Hulet lab when they were considering using an iodine cell to lock their $\SI{646}{\nm}$ laser for a \Li{6} UV MOT \cite{Duarte2011.PRA.84.061406}.}.
A longer cell and a more sensitive detection method may be necessary since most reported {\tsup{127}I\tsub{2}} spectroscopy studies in this wavelength range utilized cells longer than $\SI{30}{\cm}$ in addition implementing some sort of modulation spectroscopy to improve the signal-to-noise ratio (e.g., see \cite{Sansonetti1997.JOSAB.14.1913, Huang2018.AO.57.2102, Onishchenko2019.PRA.99.052503}).
...
Although the iodine spectroscopy was unsuccessful, we were able to calibrate the wavemeter by measuring the wavelengths of lasers locked to atomic transitions in \Sr{88} ($\SI{689}{\nm}$) and in \Li{6} ($\SI{671}{\nm}$ and $\SI{323}{\nm}$).
The $\SI{689}{\nm}$ reference comes from the red master laser locked to the $\SLJ{1}{S}{0} \rightarrow \SLJ{3}{P}{1}$ in \Sr{88} in the Killian laboratory for strontium experiments.
Ya-Ting Chang, Danyel Cavazos, and Dr. Randy Hulet were kind enough to let us run a fiber\footnote{Also a Thorlabs SM600 FC/PC SM fiber.} between the Killian and Hulet laboratories in order to measure the wavelengths of their $\SI{671}{\nm}$ and $\SI{323}{\nm} = 2 \times \SI{646}{\nm}$ lithium cooling lasers.
...
In addition to keeping the wavemeter in-place, the wavelength calibration measurements were performed within about $\SI{2}{\hour}$ to reduce environmental influences. 
\begin{table}[!htbp]
	\caption{
		\label{tab:wavemeter_calibration}
		Values used to calibrate the WA-1500 on a single day (2018/03/09).
		All measurements were performed within about $\SI{2}{\hour}$. 
		The value of the $\SI{689}{\nm}$ transition in \Sr{88} were taken from \cite{Ferrari2003.PRL.91.243002, Sansonetti2010.JPCRD.39.033103}.
		The values of the $\SI{671}{\nm}$ and $\SI{323}{\nm} = 2 \times \SI{646}{\nm}$ transitions and hyperfine splittings in \Li{6} were taken from \cite{Sansonetti2011.PRL.107.023001, Sansonetti2012.PRL.109.259901, Radziemski1995.PRA.52.4462}.}
	\centering
	\makebox[\textwidth][c]{
	\begin{tabular}{@{}ccccc@{}}
		\toprule
...
Quantum defects were obtained from the \Sr{87} data by first subtracting the calculated hyperfine energy shift to obtain an effective $I = 0$ Rydberg state energy, and then applying \cref{eq:ene0} with the mass-scaled Rydberg constant $R\pqty*{m_{87}} = \SI{109736.6230135+-0.0000007}{\per\cm}$ (see \cref{sec:rydberg_properties}) and ionization limit $E^{\pqty*{0}}_{\text{ion}} = \SI{45932.1956}{\per\cm}$.
\begin{figure}[!htbp]
	\centering
	...
	\caption{
		...
		The quantum defects derived from the \Sr{87} data were obtained after removing the hyperfine shift.}
\end{figure}
\begin{figure}[!htbp]
	\centering
