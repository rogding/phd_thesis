...
\begin{itemize}
	...
	The in-vacuum electric field plates are mounted to the bottom flange which has eleven \SI{10}{\kV} SHV feedthroughs for high-voltage connections.
...
Two pairs produce fields in the horizontal plane parallel to the table surface and are oriented such that the ``X-axis'' produce a field towards or away from the MCP whereas the ``Y-axis'' produces a field perpendicular.
...
Both the horizontal (X-axis and Y-axis) trim coils are wound on aluminum U-channel and held together with polyimide tape.
** MORE DETAILS?? **
...
One of the things we did differently from the previous strontium experiments is that we have two pairs of MOT coils instead of a single pair (see \cref{fig:dual_mot_coils}): the larger pair is used for the blue MOT while the smaller pair for the red MOT.
