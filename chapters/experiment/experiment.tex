...
\begin{figure}[htbp]
	...
	\caption{
		...
		Overview of the strontium Rydberg experiment vacuum system highlighting the main sections of the apparatus and various components.}
\end{figure}
We follow a conventional setup for producing cold and ultracold strontium gases (e.g., see \cite{Boyd2007.PhD, Stellmer2013.PhD, Barker2016.PhD}) where our system can roughly be divided in to three main sections:
\begin{description}
	\item[Source-side vacuum system]
		The starting point where an oven heats solid strontium to about \SI{425}{\celsius}, producing an effusive beam of strontium atoms directed down the Zeeman slower and towards the main chamber.
		A two-demensional (2D) collimator stage which uses transverse optical molasses to increase the flux of atoms down the Zeeman slower.
	\item[Zeeman slower]
		A Zeeman slower used to reduce the atomic velocities from the oven to something capable of being captured by the blue magneto-optical trap (MOT).
	\item[Experiment-side vacuum system]
		This is where most of the magic happens with atoms from the Zeeman slower being laser cooled and captured ready for experiments. 
\end{description}
...
To facilitate vertical optics (e.g., vertical MOT beams), our table came with a \SI{10}{\inch} hole under the main chamber.
...
This section is where strontium is converted from its solid metallic form in to a gaseous form. 
...
We've observed an increase in blue MOT loading rate by about ** 8 times ** when the 2D collimator is well aligned to the beam.
...
A shutter and a gate valve is also installed between the source-side vacuum and the Zeeman slower. 
...
In principle, the gate valve should allow us to isolate the source-side vacuum system from the rest of the chamber but we found it to be somewhat leaky when we initially pumped out our system\footnote{Something similar was noticed by the Weld group at UCSB and they used two gate valves to separate their atomic source from their UHV main chamber \cite{Senaratne2018.PhD}.}.
...
The Zeeman slower was constructed in multiple layers as the resulting field was tweaked to match the desired profile.
Each layer being attached to the Zeeman slower tube with thermally-conductive epoxy ** GIVE MODEL **. 
...
The experiment-side vacuum system consists of two primary sections:
\begin{description}
	\item[Main chamber]
		This is where the magic happens where strontium is laser cooled and trapped for performing experiments. 
	\item[Pumping tower]
		High-conductance fittings between the main chamber and the vacuum pumps to maximize pumping speed.
\end{description}

Francisco designed the main chamber which was custom fabricated\footnote{Huntington Mechanical Labs} and features
\begin{description}
	\item[Recessed top and bottom flanges]
		...
		The in vacuum electric field plates are mounted to the bottom flange which also features \num{11} \SI{10}{\kV} SHV feedthroughs.
...
We currently have three blue diode-based lasers which generate all the \SI{461}{\nm} power available on our experiment.
A single external cavity diode laser (ECDL) ``master'' laser is locked to a homebuilt strontium saturated absorption cell and injection locks two slave lasers.

Our current master laser is a New Focus Vortex Plus TLB-6802 which provides about ** \SI{40}{\mW} ** of output power.
...
We also noticed some weird grounding issues with the TLB-6700 drivers New Focus sent us with the lasers\footnote{Similar issues were seen by Jun Ye's group when I asked them if they have observed issues with the New Focus lasers.}.
