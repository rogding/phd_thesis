...
A homebuilt voltage-controlled current driver\footnote{Powered by an APEX PA12(A) power operational amplifier.} but we should be able to use any current driver capable of supplying up to about $\SI{5}{\ampere}$. 
...
In a master-slave setup, a single (typically low-power) ``master'' external cavity diode laser (ECDL) is frequency stabilized relative to the atomic transition and is used to injection lock higher-power ``slave'' lasers to amplify the laser power \cite{Shimada2013.RSI.84.063101, Hosoya2015.RSI.86.073110}.
...
While the laser was undergoing repairs, we changed our setup so that the two slave lasers are injection locked from the output of a single-mode polarization maintaining (PM) fiber so that we can easily change master lasers should this laser die again.
We also noticed some weird grounding issues with the New Focus TLB-6700 drivers included with the lasers\footnote{Similar issues were mentioned by Sara Campbell in Jun Ye's group when I asked them if they had issues with the New Focus lasers.}.
We also have concerns about the long-term cost-effectiveness and reliability of the New Focus ECDL\footnote{The piezo required installation of a new diode, even when the existing diode was fine, and the initially quoted cost was 11,500. This was significantly reduced after extended discussions with the company.}, and we have purchased a Toptica $\SI{461}{\nm}$ ECDL system\footnote{We ended up purchasing a \SI{461}{\nm} ECDL from Toptica but both companies provided quotes for NEW lasers, about 21k from MOGLabs and about 25k from Toptica but with about \SI{100}{\mW} of output power.} as a future replacement and to enable additional experiments. 
...
In principle, it should be possible to lock the frequency of the $\SI{481}{\nm}$ laser using the $\nSLJ{5s5p}{3}{P}{2} \rightarrow \nSLJ{5p^2}{3}{P}{2}$ absorption feature in a hollow cathode lamp (HCL) due to collisions populating enough metastable atoms to perform spectroscopy on \cite{Norcia2016.RSI.87.023110}.
...
The drawbacks is that we're much more sensitive to misalignments, and the non-Gaussian intensity profiles out of the diodes lead to strange red-MOT shapes. 

We currently have three slave lasers that amplify the light from the red master. 
...
\begin{itemize}
	\item Koheras Basik BMY10PztSPm, KOH2038 --
		The narrowest $\SI{640}{\nm}$ lines observed with this seed are around ** $\SI{1}{\MHz}$ **, likely limited by the slow feedback bandwidth of the piezo element, but it's capable of tuning multiple gigahertz so it's preferred for experiments requiring large tuning.
...
In-vacuum electric field plates were installed to mitigate stray electric fields at the location of the atoms as well as to provide the capability for trimming residual electric fields and perform selective field ionization (SFI).
The electric field plate system was designed by Francisco Camargo and additional details are in his Ph.D. thesis \cite{Camargo2017.PhD} and the design follows similar ones described in \cite{Millen2011.PhD, Low2012.JPB.45.113001} using a split-ring electrode geometry with four quadrant electrodes above and four quadrant electrodes below the center of the chamber (i.e., where the atoms are). 
...
An overview of how the charted particle detection system works is shown in \cref{fig:charged_particle_detection}.
The following procedure explains how electrons are detected but it can be configured to detection ions instead.
After performing a Rydberg excitation on a sample, an electric field ramp is applied where the plates closer (further) from the MCP are ramped to positive (negative) high-voltage to push electrons towards the MCP. 
...
\end{figure}
The high-voltage supplies for the electric field plate ramps are adjusted depending on which principal quantum number ($n$) of the Rydberg state being excited.
...
\begin{figure}[htbp]
	...
	\caption[]{
		...
		Connections are made using beryllium-copper inline barrel connectors with ceramic washers were placed between the screws to act as a spacer to keep the connections from touching each other and the main vacuum chamber.
...
\begin{equation}
	y	=	A \qty[1 - \exp(B x)]
\end{equation}
determines the constants $A$ and $B$, allowing for correction of undercounting in the non-linear regime. 