...
This chapter describes the Rydberg apparatus used in this thesis with the purpose of providing an overview of the current state of various systems for producing ultracold and quantum degenerate gases of strontium as well as providing references that may be helpful for future upgrades. 
Being the third ultracold strontium experiment at Rice, we leveraged lessons learned from the other experiments, colloquially referred to as ``Neutral'' and ``Plasma'', to build a capable and flexible system\footnote{Collectively, the three experiments work with ultracold atomic strontium in all forms: from the ground state (Neutral) to the ions (Plasma), and everything in between (Rydberg). There is also the strontium atomic beam experiment, which produces some of highest $n$ Rydberg atoms ever reported.}.

Although the Rydberg system looks very similar to other ultracold strontium experiments, what makes it uniquely suited for studying Rydberg physics is the inclusion of systems for creating, manipulating, and detecting charged particles. 
In particular, in-vacuum electric field plates can both cancel stray electric fields as well as apply field ionization ramps and direct the resulting electrons (or ions) towards a microchannel plate (MCP) detector.
...
\begin{figure}[!htbp]
	\centering
	\includesvg[keepaspectratio, width=5in, height=\textheight]{experiment/strontium_level_diagram/Sr_Energy_Level_Diagram-Boson.svg}
	\caption[]{
...
\begin{description}
	\item[Cooling and trapping on the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ transition] \hfill \\
		The broad ${\Gamma}/{2\pi} = \SI{30.5}{\MHz}$ transition at $\SI{461}{\nm}$ is used for the first stage of cooling due to its ability to scatter many photons very rapidly, exerting large radiative forces on the atoms. 
	...
	\item[Cooling and trapping on the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{3}{P}{1}$ transition] \hfill \\
		...
		Due to the narrower linewidth, the $\SI{689}{\nm}$ transition scatters photons much more slowly than the $\SI{461}{\nm}$ transition, making it unsuitable for capturing large numbers of atoms from the strontium oven.
		However, it is well-suited for bridging between the millikelvin temperatures of the blue MOT and the microkelvin trap depths of optical traps.
		A $\SI{689}{\nm}$ ``red'' MOT easily produces samples with temperatures around $\SIrange[range-phrase=-]{1}{2}{\micro\K}$, ideal for efficient loading into optical traps.
	\item[Optical trapping and evaporative cooling] \hfill \\
		Further cooling requires evaporative cooling in a conservative trap.
		Optical dipole traps (ODTs) are used to create a conservative potential for strontium because ground-state strontium atoms have no magnetic moment and, therefore, cannot be magnetically trapped. 
		Optical dipole traps work by inducing energy shifts of atomic states via off-resonantly coupling and, therefore, require high powers to achieve sufficient trap depths. 
		As a result, lasers at $\SI{532}{\nm}$ or $\SI{1064}{\nm}$ are commonly employed.
		Once cold atoms from the red MOT are loaded in to an ODT, evaporative cooling can be employed to produce ultracold and quantum degenerate gases with temperatures well below $\SI{1}{\micro\K}$.
\end{description}
Detailed explanations of the various techniques and a variety of experimental setups are available from numerous sources (e.g., see \cite{Boyd2007.PhD, Mickelson2010.PhD, Martinez2012.PhD, Millen2011.PhD, Stellmer2013.PhD, Boddy2014.PhD, Barker2016.PhD, Campbell2017.PhD}).
...
Although oven temperatures vary from setup-to-setup, they are typically around $\SIrange[range-phrase=-]{450}{750}{\celsius}$\footnote{If the setup has a nozzle heater, its temperature is generally higher than the main oven to reduce the likelihood of clogging.} \cite{Boyd2007.PhD, Ludlow2008.PhD, Schioppo2012.RSI.83.103101, Stellmer2013.PhD, Camargo2015.MS, Ding2016.MS, Barker2016.PhD, Campbell2017.PhD, Senaratne2018.PhD}. 
...
\Cref{tab:blue_isotope_shift} gives the isotope shifts of the $\SI{461}{\nm}$ transitions referenced to \Sr{88}\footnote{As the most abundant isotope, \Sr{88} generally produces the largest signals for laser stabilization purposes.}.
\begin{table}[!htbp]
	\caption[]{
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		A picture of the \Sr{88} blue MOT taken on 2014/08/29 through the $\SI{2.75}{\inch}$ absorption imaging viewport.
		The strontium oven and Zeeman slower are located off to the right.
		Considering the top and bottom electric field plates separated by about $\SI{1}{\inch}$, this blue MOT likely has a vertical height close to $\SI{0.5}{\inch}$.}
\end{figure}
Some experiments (e.g., \cite{Barker2016.PhD}) implement a ``cold'' blue MOT stage where the detuning, intensity, and quadrupole gradient are ramped to obtain colder temperatures for improved transfer efficiency to the red MOT. 
No such stage is implemented on our current setup as we found the blue MOT temperatures sufficient for decent transfer.

Although the $\SI{461}{\nm}$ light is capable of scattering many photons per second, the Doppler limit of this transition is about $T_{D} = \SI{732}{\micro\kelvin}$\footnote{For the $\SI{461}{\nm}$ transition: $k_{B} T_{D} \equiv \flatfrac{\hbar \Gamma}{2} = \SI{732}{\micro\kelvin}$ (Doppler limit), $k_{B} T_{r} \equiv \flatfrac{\hbar^2 k^2}{2 M} = \SI{512}{\nano\kelvin}$ (recoil limit).} \cite{Metcalf1999.LCT} with typical temperatures achieved in experiments closer to $\SI{1}{\milli\K}$.
While sub-Doppler cooling is not available in the bosonic isotopes, it has been observed in the fermionic isotope \Sr{87} where blue MOT temperatures of about $\SI{300}{\micro\kelvin}$ were achieved \cite{Xu2003.PRL.90.193002, Xu2003.JOSAB.20.968}.
...
\label{ssec:magnetic_trap_and_repumping}
...
As seen in \cref{tab:blue_isotope_shift}, the isotope shifts are comparable to the $\SI{461}{\nm}$ linewidth meaning a blue MOT can only efficiently trap a single isotope at a time. 
The key to trapping multiple isotopes is to utilize the automatic accumulation of $\nSLJ{5s5p}{3}{P}{2}$ metastable atoms, which are dark to the $\SI{461}{\nm}$ cooling light, in the magnetic trap.
...
This sequential loading process typically starts with the most abundant isotope, since those atoms will spend the most time the magnetic trap, before tuning the $\SI{461}{\nm}$ laser to cool and trap subsequent isotopes. 
Loading rates into the magnetic trap could be improved by implementing a ``depumping'' laser that prevents $\nSLJ{5s5p}{3}{P}{1}$ atoms from returning to the ground state by optically pumping them to the $\nSLJ{5s5p}{3}{P}{2}$ \cite{Barker2015.PRA.92.043418}.

Before moving on to the next cooling stage, atoms in the metastable reservoir need to be returned to the $\nSLJ{5s^2}{1}{S}{0}$ ground state.
Several transitions have been explored for repumping atoms out of the metastable reservoir, each of which has varying advantages and disadvantages \cite{Courtillot2005.EPJD.33.161, Boyd2007.PhD, Ludlow2008.PhD, Barker2015.PRA.92.043418, Campbell2017.PhD, Cooper2018.PRX.8.041055, Mickelson2009.JPB.42.235001, Stellmer2013.PhD, Stellmer2014.PRA.90.022512, Moriya2018.JPC.2.125008, Mason2013.BS, Stellmer2014.PRA.90.022512, Moriya2018.JPC.2.125008, Wongwaitayakornkul2013.BS, Camargo2015.MS, Ding2016.MS, Couturier2018.RSI.89.043103, Hu2019.PRA.99.033422}\footnote{An excellent overview of the various strontium repumping transitions is presented in \cite{Stellmer2013.PhD}.}.
On our experiment, we drive the $\nSLJ{5s5p}{3}{P}{2} \rightarrow \nSLJ{5p^2}{3}{P}{2}$ transition at $\SI{481}{\nm}$.
One of the key advantages of this transition is that the upper state is $\SLJ{3}{P}{2}$ so it only requires a single laser to clear out the magnetic trap because the $\nSLJ{5p^2}{3}{P}{2}$ atoms can only decay to either the $\nSLJ{5s5p}{3}{P}{1}$ or $\nSLJ{5s5p}{3}{P}{2}$ states. 
Another advantage of this transition is that the \Sr{87} hyperfine structure of the doubly-excited $\nSLJ{5p^2}{3}{P}{2}$ state is expected to be small meaning the shifts of the $\SI{481}{\nm}$ transition should be predominantly due to the hyperfine structure of the $\nSLJ{5s5p}{3}{P}{2}$ state which is known \cite{Heider1977.PRA.16.1371, Onishchenko2019.PRA.99.052503}. 
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5in, height=\textheight]{experiment/repumper_system/20180327-preliminary_84Sr_87Sr_repumper_spectra/repumper_spectra.pdf}
	\caption[]{
		...
		The labeled arrows indicate the expected energy shifts of the $\nSLJF{5s5p}{3}{P}{2}{F} \rightarrow \nSLJ{5p^2}{3}{P}{2}$ transition in \Sr{87} calculaed using only the hyperfine $A$ and $B$ constants of the $\nSLJ{5s5p}{3}{P}{2}$ state from \cite{Heider1977.PRA.16.1371}.}
\end{figure}
...
Keeping a repumper on can result in higher initial blue MOT loading rates by preventing accumulation of atoms in the metastable reservoir, which can drastically reduce experimental cycle times. 
This approach may be of interest for experiments that benefit from high repetition rates such as in optical tweezer experiments working with \Sr{88} \cite{Cooper2018.PRX.8.041055, Norcia2018.PRX.8.041054, Hanley2018.PhD, Jackson2019.arXiv.1904.03233}.
An interesting idea is employed in \cite{Bowden2019.arXiv.1907.13429} where atoms in the magnetic trap are laser cooled using the $\nSLJ{5s5p}{3}{P}{2} \rightarrow \nSLJ{5s4d}{3}{D}{3}$ transition at $\SI{2.92}{\um}$ before being returned to the $\nSLJ{5s^2}{1}{S}{0}$ state for loading in to an optical lattice.

\subsection{Cooling on the $\SI{689}{\nm}$ ``Red'' Transition}

After the $\SI{461}{\nm}$ cooling stage, the strontium atoms are typically around $\SIrange[range-phrase=-]{1}{2}{\milli\kelvin}$ whereas efficient loading of atoms in to optical dipole traps require sample temperatures closer to $\SIrange[range-phrase=-]{1}{2}{\micro\kelvin}$.
...
\begin{table}[!htbp]
	...
	\end{tabular}
\end{table}
Following \cite{Loftus2004.PRA.70.063413}, MOTs can be characterized by the ratio of the transition linewidth ($\Gamma$) to the single photon recoil frequency shift ($\omega_R$) with typical MOTs operating in the ${\Gamma}/{\omega_R} \gg 1$ regime compared to narrow line MOTs where ${\Gamma}/{\omega_R} \sim 1$.
...
The differences in dynamics are especially apparent in the shape of bosonic red MOTs where the atoms are trapped on a thin ellipsoidal shell where the laser detuning matches the Zeeman shift from the quadrupole field \cite{Stellmer2013.PhD, Stellmer2014.arXiv.1307.0601}.
Conversely, the fermionic red MOT looks similar to ``traditional'' $\flatfrac{\Gamma}{\omega_R} \gg 1$ MOTs but it's shape is due to complications arising from hyperfine structure.
...
In order to increase the capture efficiency, the spectrum of the red MOT light is artificially broadened by frequency modulation, typically by a few megahertz, which enables addressing of multiple velocity classes of atoms from the blue MOT.
...
At the end, temperatures are typically below about $\SI{2}{\micro\kelvin}$.
Our \Sr{84} red MOT system is documented in \cite{Ding2016.MS}.

While the bosonic isotopes of strontium behave like an ideal $J=0 \rightarrow J=1$ MOT described in texts (e.g., see \cite{Metcalf1999.LCT, Foot2005.Atomic}), the hyperfine structure of \Sr{87} poses a challenge.
...
This is very different than the case for alkali atoms where the $g$-factors of the lower and upper states are comparable. 
As a result, some positions experience a restoring force whereas the forces are anti-trapping at other locations. 
...
Due to the large isotope shifts compared to the linewidth of the transition (see \cref{tab:red_isotope_shift}), red MOTs for multiple isotopes can be operated simultaneously.
...
It is during this switching time that the $\SI{481}{\nm}$ repump light is applied so all the isotopes released from the metastable reservoir are returned to the $\nSLJ{5s^2}{1}{S}{0}$ ground state.
Since the repumped atoms can be an mixture of isotopes, this ability to simultaneously laser cool and trap with the red MOT is critical.
...
A promising technique is to use SWAP cooling instead of the broadband red MOT to capture atoms from the blue MOT before transferring to a single-frequency red MOT to achieve colder temperatures \cite{Snigirev2019.PRA.99.063421}.

\subsection{Optical Dipole Trap (ODT)}

Strontium in the $\nSLJ{5s^2}{1}{S}{0}$ ground state has no magnetic moment meaning it cannot be magnetically trapped\footnote{There is interest in producing quantum degenerate gases of alkaline-earth atoms in metastable states \cite{Barker2016.PRA.93.053417}, some of which are magnetically trappable. Quantum degenerate gases of metastable atoms have been realized for helium \cite{Robert2001.Science.292.461, Santos2001.PRL.86.3459, McNamara2006.PRL.97.080404}.} and requires the use of optical dipole traps (ODTs).
Since ODTs are (generally) conservative, the trapped atoms remain at approximately the same temperature as when they are loaded (typically $\SI{\sim 1}{\micro\kelvin}$).
If further cooling is required, evaporative cooling can be performed to attain temperatures sufficient for quantum degeneracy. 

Optical dipole traps work by using intense, off-resonant light to shift atomic energy levels. 
Since the amount of shift is dependent on the intensity, a spatially-varying intensity $I\pqty{\vb{r}}$ leads to force gradient such that, when appropriately designed, will attract (or repel) atoms to the area of highest intensity.
For a two-state atom with ground state $\ket{g}$ and excited state $\ket{e}$ separated by energy $\hbar \omega_0$ and coupled by a laser at frequency $\omega$, this shift can be written as (e.g., see \cite{Grimm1999.arXiv.9902072, Steck.QuantumAtomOptics})
\begin{equation}
	\label{eq:odt_potential}
	{\Delta}E\pqty{\vb{r}}	=	-\frac{3 \pi c^2}{2 \omega_{0}^{3}} \pqty{\frac{\Gamma}{\omega_0 - \omega} + \frac{\Gamma}{\omega_0 + \omega}} I\pqty{\vb{r}}
					=	-\frac{1}{2 \epsilon_0 c} \Re\bqty{\alpha\pqty{\omega}} I\pqty{\vb{r}}
\end{equation}
...
A quick and easy way to generalize \cref{eq:odt_potential} to a multilevel atom is to treat the multiple levels as individual two-level systems and then sum the contributions due to off-resonant coupling in each two-level system (e.g., \cite{Katori1999.JPSJ.68.2479, Boyd2007.PhD, Ye2008.Science.320.1734}). 
A more accurate calculation of the AC Stark shift would require diagonalizing the Hamiltonian which includes all the states in the atom coupled by the laser.
...
For strontium, the red MOT spatial distribution and the particular isotope(s) being trapped needs to be taken in to account when designing an ODT.
While the fermionic red MOT traps atoms in an ellipsoidal volume around the MOT quadrupole center, the atoms in a bosonic red MOT tend to form a thin shell below the quadrupole center \cite{Stellmer2013.PRA.87.013611, Stellmer2014.arXiv.1307.0601}. 
As a result, improved loading of atoms into an ODT can be obtained by ``mode-matching'' the trapping volume of the ODT to the spatial distribution of atoms in the red MOT.
For strontium, the preferred ODT geometry appears to be a high-aspect ratio horizontal ``pancake'' or ``light sheet'' where the tight axis is along gravity as this matches well with the shape of the bosonic red MOT \cite{Stellmer2013.PRA.87.013611, Stellmer2013.PhD, Barker2016.PhD, Oppong2015.MS, Campbell2017.PhD}. 
...
Single-beam traps typically provide very weak confinement along the beam so secondary vertical or horizontal ODT beams are generally added to increase confinement along weak axes. 

The specific isotope(s) being trapped also have an effect on both loading into an ODT as well as on their evaporation due to their wildly varying intra- and inter-isotope scattering lengths.
\Cref{tab:sr_scattering_lengths_short} lists the $s$-wave scattering lengths for various combinations of isotopes. 
\begin{table}[!htbp]
	\centering
	\caption[]{
		\label{tab:sr_scattering_lengths_short}.
		Measured $s$-wave scattering lengths ($a_s$) of strontium for various pairs of isotopes given in $\si{\bohr}$ from \cite{Stein2010.EPJD.57.171}.
		An extended version is given in \cref{tab:sr_scattering_lengths_long}.}
	\begin{tabular}{@{}ccccc@{}}
		\toprule
	...
	\end{tabular}
\end{table}
The first strontium BECs were achieved with \Sr{84}, which has a convenient $a_s = \SI{123}{\bohr}$ scattering length, was easily evaporated to quantum degeneracy in low aspect ratio traps \cite{Martinez2009.PRL.103.200402, Stellmer2009.PRL.103.200401}.
...
An overview of our vacuum system is shown in \cref{fig:vacuum_system} with the some of the major components labeled.
\begin{figure}[!htbp]
	\centering
...
\end{figure}
Perhaps the most distinctive aspect of our vacuum system is that it was designed to have the majority of the horizontal laser beams about $\SI{3.5}{\inch}=\SI{8.89}{\cm}$ from the surface of the optical table.
Keeping the beams at this height eliminated the need for rigid supporting structures to elevate the entire vacuum system along with the associated requirement for raised optical platforms.
This should also improve beam pointing stability as there is reduced need for periscopes to raise laser beams to the level of the vacuum chamber. 
...
The major disadvantage of our current setup is the long distance between the undertable breadboard and the bottom of the main chamber, about $\SI{12}{\inch}$\footnote{The thickness of the optical table.}, which makes it difficult to design, install, and align sensitive vertical beams launched from below.
...
Although there are various designs for strontium ovens \cite{Senaratne2015.RSI.86.023105, Schioppo2012.RSI.83.103101} (and even commercial systems), we used a simple design that has shown success on the older strontium experiments in the laboratory \cite{Martinez2012.PhD, Camargo2015.MS, Aman2019.PhD}.
...
An array of capillary tubes form a nozzle to provide some degree of collimation and resistive heater wire is wrapped around them to reduce the effects of strontium building up in the capillary tubes and clogging.
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		Oven without (left) and with (right) the heat shield.
		Some additional details can be found in \cite{Martinez2012.PhD, Camargo2015.MS, Aman2019.PhD}.}
\end{figure}
The larger flange provides extra room for various feedthroughs without being too cramped.
...
To maximize the effectiveness of the limited $\SI{461}{\nm}$ laser power, the beams have elliptical intensity profiles to maximize the intensity on the atomic beam and a single beam is recycled through the four arms of the 2D collimator.
...
Before the atoms leave this section of the vacuum system and enter the Zeeman slower, they pass through a differential pumping tube\footnote{A cylinder of oxygen-free high thermal conductivity (OFHC) copper with a coaxial through hole with approximate dimensions of $\diameter = \SI{0.3}{\inch}$ by $L = \SI{2.4650}{\inch}$.} with a conductance $C \approx \SI{0.85}{\liter\per\second}$ that helps maintain the pressure differential between the source and experiment sides of the system. 
The pressure in the source side is typically higher than the experiment side and is currently around $\SI{7.6E-9}{\Torr}$\footnote{MiniVac ion pump controller outputting $\SI{7.9}{\mV}=\SI{7.9E-6}{\A}$ for the  rebuilt VacIon Plus 75 Starcell on 2019/06/13.}.
...
The shutter allows us to physically block the atomic beam and extend the time between needing to change the entrance window for the $\SI{461}{\nm}$ Zeeman slower beam. 
The gate valve should, in principle, allow us to isolate the source-side vacuum system from the rest of the chamber to facilitate reloading the strontium oven without venting the entire vacuum chamber but we found this valve to be somewhat leaky\footnote{Something similar was noticed by the Weld group at UCSB and they used two gate valves to separate their atomic source from their UHV main chamber \cite{Senaratne2018.PhD}.} during our initial pump down.
...
The Zeeman slower uses a spatially-varying axial magnetic field to keep the atoms resonant with a circularly-polarized $\SI{461}{\nm}$ beam propagating counter to the direction of the atoms so that they continuously scatter photons as they slow \cite{Phillips1982.PRL.48.596}.
Our particular design is a ``spin-flip''-type where the amplitude of the axial magnetic field crosses zero between the ends.
...
Each layer was affixed to the Zeeman slower with thermally-conductive epoxy\footnote{Emerson and Cuming STYCAST 2762 or STYCAST 2762 FT with Catalyst 14, 17, or 17M-1.} and the resulting magnetic field measured before wrapping the next layer.
Due to its length\footnote{The inner wall of the Zeeman slower is about $\diameter = \SI{0.6940}{\inch}$ by $L = \SI{18.8296}{\inch}$.}, it's conductance is reduced ($C \approx \SI{1.4}{\liter\per\second}$) which should help maintain the differential pressure between the source and experiment sides.

For cooling, the Zeeman slower was custom made with a double wall that acts as a water cooling jacket.
The inner wall separates the cooling water from UHV and the magnetic field coils are wrapped around the outer wall.
...
The experiment-side vacuum system can be broken down in to the main chamber section where experiments take place and a ``pumping tower'' section which provides a high-conductance path to the vacuum pumps.
The custom main chamber\footnote{Fabricated by Huntington Mechanical Labs.} features:
\begin{itemize}
	\item Recessed top and bottom flanges which bring the $\SI{2.75}{\inch}$ viewports closer to the center of the chamber while providing room for the MOT coils to fit around the viewports.
	The in-vacuum electric field plates are mounted to the bottom flange which has eleven $\SI{10}{\kV}$ safe high voltage (SHV) feedthroughs for the electric field system.
	\item Six (6) $\SI{2.75}{\inch}$ horizontal viewports provide optical access for the various trapping beams (e.g., MOT and ODT) and the imaging system.
	\item Three (3) $\SI{1.33}{\inch}$ ``mini'' horizontal viewports used for the various $\SI{689}{\nm}$ and $\SI{320}{\nm}$ excitation beams.
	These are oriented perpendicular and parallel to the MCP, allowing us to apply bias magnetic fields that set the quantization axis along the MCP axis to reduce electron deflection by the Lorentz force.
	\item Room for a Photonis miniTOF micro-channel plate (MCP) detector to detect charged particles.
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		Transmission data of the $\SI{1.33}{\inch}$ and $\SI{2.75}{\inch}$ viewports with the anti-reflection (AR) coating by Reynard Corporation\protect\footnotemark.
		Although the AR coating was specified for $\SI{461}{\nm}$, $\SI{689}{\nm}$, and $\SI{1064}{\nm}$, it's fortuitous that the coating should be very good for the $\SI{698}{\nm}$ clock transition and acceptable for $\SI{532}{\nm}$ and $\SI{813}{\nm}$ should we ever decide to use traps at those wavelengths.}
\end{figure}
...
In retrospect, we should have also installed a titanium sublimation pump (TSP) for additional pumping capability.
...
A $\SI{3.375}{\inch}$ gate valve separates the Zeeman slower viewport from the main chamber to facilitate changing this viewport once it gets coated with strontium.

\section{Magnetic Field Trim Coils}

Three pairs of Helmholtz coils are used to cancel stray magnetic fields and to apply a bias field and are shown in \cref{fig:trim_coils}. 
The $X$-axis and $Y$-axis coil pairs produce fields in the horizontal plane parallel to the table surface and are oriented such that the $X$-axis produces a field towards or away from the MCP whereas the $Y$-axis produces a field perpendicular to this axis.
...
Both the horizontal ($X$-axis and $Y$-axis) trim coils are constructed from {$\num{17.5}$ AWG} wire\footnote{MWS Wire Industries 17.5 HAPT-200 (NEMA MW 35-C).} with \num{45} turns wound on aluminum U-channels and compressed together with polyimide tape.
Due to the choice of keeping the center of the chamber at $\SI{3.5}{\inch}$ above the table surface, this required the horizontal ($X$-axis and $Y$-axis) trim coils to be centered at this height leading to their rectangular geometry\footnote{It should be noted that the $X$ and $Y$ trim coils do not have identical dimensions. They are construced from the same aluminum U-channel and have the same height but are slightly different widths.} instead of square or circular.

The vertical ($Z$-axis) coils were wound on circular aluminum U-channel forms and fit around the outside of the top and bottom flanges of the main chamber.
This geometry allowed us to put $\num{76}$ turns of {$\num{14}$ AWG} rectangular wire\footnote{MWS Wire Industries 14 SQ HML-240 (NEMA MW 20C).} on the forms.
The circular U-channel forms also feature a radial slice to mitigate the generation of eddy currents. 
In retrospect, we should made the horizontal coils beefier (especially along the $X$-axis) in order generate larger bias fields along the MCP axis.
...
One of the things we did differently from the previous strontium experiments is that we have two pairs of MOT coils instead of a single pair where the larger pair is used for the blue MOT while the smaller pair is used for the red MOT (see \cref{fig:dual_mot_coils}).
This allows us to use two different current drivers without needing a single supply with enough dynamic range to supply both the $\SI{\sim 40}{\ampere}$ to generate the $\SI{\sim 40}{\gauss\per\cm}$ gradient for the blue MOT and also generating a low-noise $\SI{\sim 1}{\ampere}$ for the $\SI{\sim 1}{\gauss\per\cm}$ gradient for the red MOT.
\begin{figure}[!htbp]
	\centering
	...
	\caption[]{
		...
		(Left) Picture of our assembled dual MOT coils on a test setup for measuring their magnetic field gradients. 
		(Right) CAD rendering of how they are centered around the windows of the recessed flanges on the main chamber.}
\end{figure}
Both pairs of coils share a common vertical axis.
A drawing of the dual MOT coils can be found in \cref{ap:dual_MOT_coils}.

The blue MOT coils are constructed from square tubular wire\footnote{S{\&}W Wire 125SQ DPG/BARE (NEMA 46-C).} which is hollow inside for cooling water and produce a magnetic field gradient of about $\SI{1.25}{\gauss\per\cm\per\ampere}$ along the tight axis (the $Z$-axis) (see \cref{fig:dual_mot_coil_fields}).
The coils are typically run with around $\SIrange[range-phrase=-]{40}{50}{\ampere}$ from a power supply\footnote{TDK-Lambda Genesys 8-90-IS510-U which can supply up to $\SI{8}{\volt}$ and $\SI{90}{\ampere}$.} set for fixed-current output.
A power MOSFET\footnote{An IXYS IXFN 340N07.} is used for quickly switching the current on and off.
...
The red MOT coils produce a magnetic field gradient of about $\SI{1}{\gauss\per\cm\per\ampere}$ (see \cref{fig:dual_mot_coil_fields}).
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		The uncertainties are estimated to be $\pm \SI{0.25}{\cm}$ and $\pm \SI{0.05}{\gauss}$.
...
In a master-slave setup, a single (typically low-power) frequency-stabilized ``master'' external cavity diode laser (ECDL) is used to injection lock higher-power ``slave'' lasers to increase the available laser power \cite{Shimada2013.RSI.84.063101, Hosoya2015.RSI.86.073110}.
The blue master laser is locked relative to the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ transition in a homebuilt strontium cell using Doppler-free saturated absorption spectroscopy.
...
Although this laser generally remains single-mode when tuning between the various isotopes, we have been very disappointed with output mode quality which appears to be a Hermite-Gaussian {TEM\tsub{10}} (or higher order) as seen in \cref{fig:VP-0024_output}.
\begin{figure}[!htbp]
	\centering
	...
	\caption[]{\label{fig:VP-0024_output}
		Output beam profile from the New Focus TLB-6802 (VP-0024) $\SI{461}{\nm}$ master ECDL.
		Although this image was taken after we received the laser back from repair, it still exhibits a similar output mode as when we first received the laser.}
\end{figure}
...
While the laser was undergoing repairs, the injection locking setup was modified so that the two slave lasers are now injection locked from the output of a single-mode polarization maintaining (SMPM) fiber so that we can easily change master lasers should this laser die again.
We also noticed some strange grounding and noise issues with the current master laser system which seemed very sensitive to the particular cables connecting the feedback circuit to the laser driver\footnote{Similar grounding and noise issues with the New Focus laser was mentioned by Sara Campbell in Jun Ye's group. She documents their experiences in \cite{Campbell2017.PhD}.}.
We also have concerns about the long-term cost-effectiveness and reliability of the New Focus ECDL\footnote{During the piezo repair, New Focus recommended installation of a new diode and the initially quoted cost, for both the repair and new diode, was about half that of buying a new ECDL system. This was significantly reduced after extended discussions with the company.} and have purchased a Toptica $\SI{461}{\nm}$ ECDL system\footnote{The Toptica $\SI{461}{\nm}$ ECDL is capable of outputting about $\SI{100}{\mW}$.} as a future replacement and to enable additional experiments. 
...
Currently, we use two New Focus TLB-6802-IJ\footnote{From what we can tell, these are simply their standard TLB-6802 without the grating and probably using the Nichia NDB4216E non-AR coated diode which is specified to output about $\SI{100}{\mW}$.} as the slave lasers, both of which output about $\SI{100}{\mW}$ of power.
...
Mechanical shutters\footnote{Although bulky, hard drive shutters are used because we've found them to be a cheap and reliable solution.} are used to provide $\SI{100}{\percent}$ extinction.
...
The current $\SI{481}{\nm}$ repumper system is shown in \cref{fig:repumper_system} with light provided by a Toptica DL 100\footnote{This ECDL is equipped with a {LD-0488-0060-1} diode.} which outputs about $\SI{14}{\mW}$ of power that is shared between the Rydberg, Neutral, and Plasma experiments.
\begin{figure}[!htbp]
	\centering
	...
	\caption[]{
		...
		A portion of the light form the $\SI{481}{\nm}$ external cavity diode laser (ECDL) is used to lock to a Doppler-broadened \Te{130}{2} line in a cell heated to about $\SI{555}{\celsius}$ (see \cite{Wongwaitayakornkul2013.BS}).
		The rest of the light is sent through a free-space electro-optic modulator (EOM) at about $\SI{565}{\MHz}$ which applies multiple sidebands to the light in order to address the various isotopes and hyperfine states.
		The $\SI{481}{\nm}$ light is delivered to the various experiments by multimode (MM) fibers.}
\end{figure}
...
Now that we routinely work with multiple isotopes, a free-space electro-optic modulator (EOM)\footnote{Qubig EO-565M3-VIS\_T\_TXC.} was introduced to add sidebands to the $\SI{481}{\nm}$ light in order to address the multiple bosonic isotopes and the hyperfine shift of \Sr{87}.
...
For most of the experiments described in this thesis, the red master laser was found to have a linewidth of about $\SI{30}{\kHz}$\footnote{The Toptica ECDL master is likely much narrower and we suspect the atom lock to be broadening the laser to about $\SI{30}{\kHz}$.}.
...
\begin{description}
	\item[$f_\text{master}$] \hfill \\
		...
		Slave lasers injection locked at this frequency are primarily used for both trapping and spectroscopy of the bosonic isotopes. 
		This light is also used to produce the \Sr{87} red MOT ``stir'' beam and for spectroscopy utilizing the $\nSLJf{5s^2}{1}{S}{0}{{9}/{2}} \rightarrow \nSLJf{5s5p}{3}{P}{1}{{9}/{2}}$ transition.
...
A rejected beam from the isolator is used to monitor the frequency modes of the slave lasers\footnote{In the future, I would recommend using a stray reflection or beam sampler after the isolator instead as the current arrangement could lead to a small amount of light from the spectrum analyzer cavity getting back to the slave lasers (especially since multiple slave lasers share the spectrum analyzer cavity) and possibly leading to laser instability.}.
...
Since the red MOT is only operated for a few hundred milliseconds, the zeroth-order from the red MOT AOMs also provide the light for various other purposes as well (e.g., spectroscopy, loss, etc.). 
\begin{sidewaysfigure}[!htbp]
	\centering
...
The current setup operates at $\SI{1064}{\nm}$ with the high powers provided by a Nufern NuAMP SUB-1382 fiber amplifier which amplifies a few milliwatts of seed light up to about $\SI{50}{\W}$.
...
These fibers are air-gapped and feature an endcap on the ends that allows the beam to expand before it enters and exits the fiber, therefore reducing the power density at the fiber tip.
The adjustable fiber tip position enables optimization of the coupling efficiency.
Although the fiber tips are very delicate, we routinely achieve $\SI{> 7}{\W}$ of output power from these fibers.

The current main ODT is comprised of a single high-aspect-ratio ``sheet'' trap with beam waists of about $\SI{28}{\um}$ (vertical) by $\SI{264}{\um}$ (horizontal).
A beam profile is shown in \cref{fig:sheet_trap_profile}.
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5in, height=\textheight]{experiment/odt_system/sheet_trap/sheet_trap_profile/sheet_profile.pdf}
	\caption[]{
...
\end{figure}
The horizontal trap frequency in the main ODT is increased with a circular ODT beam with waists of about $\SI{70}{\um}$ intersecting the sheet trap at a near vertical angle.
The measured beam profile of the vertical trap is shown in \cref{fig:vertical_trap_profile}.
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5in, height=\textheight]{experiment/odt_system/vertical_trap/vertical_trap_profile/vertical_profile.pdf}
	\caption[]{
		\label{fig:vertical_trap_profile}
		Beam profile of the vertical trap from which we fit to obtain the beam waists.
		For axis 1: $w = \SI{74+-4}{\um}$, $z_{0} = \SI{36.21+-0.10}{\cm}$, and $M^2 = \num{1.02+-0.06}$.
		For axis 2: $w = \SI{68+-6}{\um}$, $z_{0} = \SI{36.58+-0.11}{\cm}$, and $M^2 = \num{1.05+-0.08}$.
		A $\pm \SI{1}{\mm}$ uncertainty was assumed in the measured distance and a $\pm \SI{5}{\um}$ uncertainty in the measured beam size.}
\end{figure}
For each experiment, we measure the trap oscillation frequencies for the sample of interest. 
...
Once the Rydberg atoms are created, they are detected by applying an electric field ramp that ionizes any Rydberg atoms and directs the resulting electrons towards a micro-channel plate (MCP) detector for counting.
...
A schematic of the $\SI{320}{\nm}$ UV laser system is shown in \cref{fig:uv_laser_system}.
\begin{figure}[!htbp]
	\centering
	\includesvg[keepaspectratio, width=\textwidth, height=\textheight]{experiment/uv_system/uv_system.svg}
	\caption[]{
		\label{fig:uv_laser_system}
		Outline of the system for generating $\SI{320}{\nm}$ radiation.
		Starting with a narrow linewidth $\SI{1064}{\nm}$ seed laser, a fiber amplifier boosts the power to several watts.
		The $\SI{1064}{\nm}$ light then pumps an optical parametric oscillator (OPO) modified with a sum frequency generation (SFG) stage to produce $\SI{640}{\nm}$ light.
		Most of the $\SI{640}{\nm}$ output power goes to a commercial second harmonic generation (SHG) system that frequency doubles the light to $\SI{320}{\nm}$.
		A small portion of the $\SI{640}{\nm}$ output is split between the wavemeter for determining the $\SI{320}{\nm}$ photon energy and the transfer cavity laser stabilization system.
		A fiber electro-optic modulator (EOM) applies tunable sidebands to the $\SI{640}{\nm}$ light before being sent to the transfer cavity. 
		Diagram of the Aculight Argos Model 2400 CW based on figures in \cite{Mieth2014.OE.22.11182, Morrison2013.RSI.84.013102}.}
\end{figure}
...
\begin{description}
	...
	\item[NP Photonics Rock RFLM-50-3-1064.53-1-S-0] \hfill \\
		The Rock's piezo feedback bandwidth is quite a bit better than the Koheras with the narrowest features observed being $\SI{\approx 150}{\kHz}$ in $\SI{640}{\nm}$.
...
The OPO section uses the $\lambda_{\text{P}} = \SI{1.064}{\um}$ pump to generate $\lambda_{\text{S}} = \SI{1.6}{\um}$ and $\lambda_{\text{I}} = \SI{3.2}{\um}$ light followed by the SFG section then combining the $\lambda_{\text{S}} = \SI{1.6}{\um}$ with another $\lambda_{\text{P}} = \SI{1.064}{\um}$ to produce $\lambda_{\text{SFG}} = \SI{640}{\nm}$.
...
The $\SI{640}{\nm}$ PDH error signal from the transfer cavity is fed back to the $\SI{1064}{\nm}$ seed laser.
...
Our typical operating powers are closer to about $\SI{100}{\milli\W}$ and varies depending on the principal quantum number we are exciting.
\begin{figure}[!htbp]
	\centering
	...
	\caption[]{
		...
		Picture of the Toptica second harmonic generation (SHG) pro bow tie doubling cavity. 
		The normally sealed SHG cavity was opened up for realigning the $\SI{640}{\nm}$ to the cavity.}
\end{figure}

\subsection{Electric Field System}

Due to the extreme $\pqty*{n^\star}^{7}$ scaling of polarizability \cite{Gallagher1994.RydbergAtoms}, Rydberg atoms are extremely sensitive to electric fields. 
In-vacuum electric field plates were installed to mitigate stray electric fields at the location of the atoms and for performing selective field ionization (SFI).
The electric field plate system, shown in \cref{fig:electric_field_system-CAD,fig:electric_field_system-real}, follows similar designs described in \cite{Millen2011.PhD, Low2012.JPB.45.113001} using a split-ring electrode geometry with four quadrant electrodes above and four quadrant electrodes below the atoms.
\begin{figure}[!htbp]
	\centering
...
\label{ssec:charged_particle_detection}
...
Some of the previous work with strontium Rydberg experiments were carried out in the Neutral apparatus but since it lacked the capability to detect charged particles, atom loss in absorption images was used instead \cite{DeSalvo2015.PRA.92.031403, DeSalvo2016.PRA.93.022709, Aman2016.PRA.93.043425}.
Our system was designed from the beginning to be able to detect charged particles have found the signal to be incredibly sensitive, providing the ability to detect signals spanning several orders of magnitude as shown in \cite{Camargo2018.PRL.120.083401}.
...
Charged particles are detected with an MCP\footnote{Photonis Advanced Performance Detector (APD) 2 miniTOF. It's important to note that we purchased the version with PEEK components so care must be taken when performing a bake.}.
...
\begin{figure}[!htbp]
	...
	\caption[]{
		...
		Dividing by the total number of loops gives the counts per bin per loop ($\num{1000}$ loops were used in the data shown above).
		The MCS detects a large ``kick'' for first several bins (red shaded region) so the data in those bins are excluded before proceeding with the analysis.}
\end{figure}

As seen in \cref{fig:mcp_saturation}, MCPs are known to ``saturate'' and become non-linear as the count rate increases.
This non-linearity is important to characterize to determine when the signal changes from the linear regime to the non-linear regime and requires the non-linearity to be taken in to account. 
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5in, height=\textheight]{experiment/mcp/mcp_saturation/20190327-Complete_data_set/mcp-counts_vs_uv_power.pdf}
	\caption[]{
...
However, data are typically only recorded under conditions where the departures from linearity are relatively small.

\section{Absorption Imaging}

Although the primary signals of interest are obtained from the charged-particle detection system, absorption imaging\footnote{Our primary absorption imaging camera is an Andor Luca-R DL-604M-OEM.} remains an important diagnostic tool for measuring the atom number and sample temperature (e.g., see \cite{Pyragius2012.arXiv.1209.3408}).
The absorption imaging process involves taking at least two images: an ``atom'' image and a ``background'' image:
\begin{description}
	\item[``Atom'' image]	\hfill \\
		The ``atom'' image is obtained by illuminating the cloud with a resonant pulse of $\SI{461}{\nm}$ light, typically about $\SI{10}{\us}$ in duration.
		Since the light is resonant with the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ transition, the atoms scatter light out of the imaging beam and the resulting shadow is recorded on the camera. 
	\item[``Background'' image]	\hfill \\
		The ``background'' image is obtained with a pulse of $\SI{461}{\nm}$ identical in duration to the ``atom'' image (ideally, with the same intensity). 
		This image provides a reference intensity map of the imaging beam without atoms and is generally taken about $\SI{100}{\ms}$ after the ``atom'' image. 
\end{description}
Once the ``atom'' and ``background'' images are obtained, the log of their ratio can be related to the density of atoms at a particular pixel which is the integrated signal along the imaging beam path (this is generally referred to as a ``column'' density). 
The sample temperature can be extracted by performing a ``time-of-flight'' measurement by introducing a time delay between releasing the atoms from the trap and the imaging pulses \cite{Yavin2002.AJP.70.149}.
In general, absorption imaging is done with an imaging beam that is far below the saturation intensity of the atomic transition ($s \ll 1$) but it is possible to perform absorption imaging at high intensities \cite{Reinaudi2007.OL.32.3143, Hueck2017.OE.25.8670}. 
Some additional details of the absorption imaging process is given in \cite{Ding2016.MS} and the references therein.

For ultracold gases of strontium, it's particularly important to note that the (on-resonance) absorption cross-section for \Sr{87} ($\sigma^{87}_{0}$) is different from the bosonic isotopes ($\sigma^{88}_{0}$).
For the bosons, the Clebsch-Gordan factor for the $\SLJ{1}{S}{0} \rightarrow \SLJ{1}{P}{1}$ is unity so the absorption cross-section is insensitive to laser polarization (at least in zero magnetic field).
This is different in \Sr{87} due to hyperfine structure where different imaging beam polarizations have different coupling strengths to the upper {$\nSLJf{5s5p}{1}{P}{1}{{7}/{2}}$, ${9}/{2}$, and ${11}/{2}$} states. 
Not only does this make absorption imaging \Sr{87} sensitive to the initial population of the various $\nSLJfM{5s^2}{1}{S}{0}{{9}/{2}}{m_F}$ substates, the $\tau = \SI{5}{\ns}$ lifetime of the $\nSLJ{5s5p}{1}{P}{1}$ state can also lead to optical pumping effects which redistributes the $m_F$ populations for an imaging pulse a few microseconds in duration.
Calculations using both a rate equation model and the full optical Bloch equations are presented in \cite{Mickelson2010.PhD, Barker2016.PhD} and show that, for a linearly polarized imaging beam, $\lim_{t_{\text{imaging}} \to \infty} {\sigma^{87}_{0}\pqty{t_{\text{imaging}}}}/{\sigma^{88}_{0}} \approx \num{0.59}$ for both spin-polarized and unpolarized samples of \Sr{87}.