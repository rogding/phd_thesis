...
We follow the standard procedure for producing cold and ultracold gases of strontium.
...
Strontium oven temperatures typically operate between $\SIrange[range-phrase=-]{425}{650}{\celsius}$\footnote{\cite{Boyd2007.PhD, Ludlow2008.PhD}: $\SI{575}{\celsius}$ (body) and $\SI{850}{\celsius}$ (nozzle); \cite{Schioppo2012.RSI.83.103101}: $\SI{450}{\celsius}$; \cite{Stellmer2013.PhD, Stellmer2013.PRA.87.013611}: $\SIrange[range-phrase=-]{600}{750}{\celsius}$; \cite{Ding2016.MS}: $\SI{425}{\celsius}$; \cite{Barker2016.PhD, Senaratne2018.PhD}: $\SI{650}{\celsius}$ (nozzle), $\SI{600}{\celsius}$ (reservoir); \cite{Campbell2017.PhD}: $\SI{500}{\celsius}$.} meaning the atoms in the beam are moving at about $\overline{v} = \SIrange[range-phrase=-]{483}{555}{\meter\per\second}$\footnote{For an atomic beam at $T = \SIrange[range-phrase=-]{425}{650}{\celsius}$: $v_{\text{mp}} = \SIrange[range-phrase=-]{445}{512}{\meter\per\second}$, $v_{\text{avg}} = \SIrange[range-phrase=-]{483}{555}{\meter\per\second}$, and $v_{\text{rms}} = \SIrange[range-phrase=-]{514}{591}{\meter\per\second}$ \cite{Metcalf1999.LCT}.}.
Since the atoms are moving so quickly, the broad $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ at $\SI{461}{\nm}$ transition is used for the pre-cooling as it's capable of scattering photons very quickly with the $\flatfrac{\Gamma_{\SI{461}{\nm}}}{2\pi} = \SI{30.5}{\MHz}$ linewidth. 

Immediately after the atoms exit the oven, they first encounter the $\SI{461}{\nm}$ two-dimensional (2D) collimator stage which applies transverse optical molasses \cite{Lett1989.JOSAB.6.2084, Joffe1993.JOSAB.10.2257} to reduce the divergence of the atomic beam although there is no effect on their longitudinal velocity.
...
At this stage, we slow the longitudinal velocity with a Zeeman slower where a spatially-varying axial magnetic field keeps the atoms in resonance as they slow so that they continuously scatter photons propagating counter to their direction of travel \cite{Phillips1982.PRL.48.596}.
...
Now that a significant portion of the atoms have been slowed by the Zeeman slower, they enter the main chamber and are cooled and trapped by a standard six-beam $\SI{461}{\nm}$ ``blue'' magneto-optical trap (MOT).
The MOT uses a quadrupole magnetic field and light detuned below atomic resonance to provide both a position-dependent and a velocity-dependent force which captures and cools the atoms \cite{Raab1987.PRL.59.2631, Metcalf1999.LCT, Foot2005.Atomic}. 
After our blue MOT, we typically have a sample of atoms around $\SIrange[range-phrase=-]{1}{2}{\milli\kelvin}$.
Although the $\SI{461}{\nm}$ is capable of scattering many photons per second, this limits the temperature it's ultimately capable of achieving with the Doppler temperature about $T_{D} = \SI{770}{\micro\kelvin}$\footnote{For the $\SI{461}{\nm}$ transition: $T_{D} = \SI{770}{\micro\kelvin}$, $T_{r} = \SI{510}{\nano\kelvin}$.} \cite{Metcalf1999.LCT}.
...
After repumping, we further cool the sample driving the narrow ($\Gamma/2\pi = \SI{7.5}{\kHz}$) $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{3}{P}{1}$ transition at $\SI{689}{\nm}$.
...
Operating in the $\flatfrac{\Gamma}{\omega_R} \sim 1$ regime where a few photon recoils can bring an atom out of resonance with the cooling lasers leads to very different dynamics.
The differences in dynamics are especially apparent in the shapes of the bosonic and fermionic red MOTs ** FIGURE? **.
...
\end{itemize}
** The \SI{2.75}{\inch} viewports were anti-reflection (AR) coated for \SI{461}{\nm}, \SI{689}{\nm}, and \SI{1064}{\nm} by Reynard Corporation. **
...
\begin{figure}[htbp]
	\includesvg[keepaspectratio, width=\textwidth, height=\textheight]{experiment/mot_coils/mot_coil_fields.svg}
