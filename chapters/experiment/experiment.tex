...
% Oven, 2D collimator, and Zeeman slower.
\subsection{Pre-cooling with the \SI{461}{\nm} transition}

Due to strontium's low vapor pressure, an oven is required to liberate strontiums atoms from the solid. 
Our oven typically operates around $\SIrange[range-phrase=-]{400}{500}{\celsius}$ meaning the atoms in the beam are moving at about $\overline{v} = \SIrange[range-phrase=-]{474}{508}{\meter\per\second}$\footnote{For an atomic beam at $T = \SIrange[range-phrase=-]{400}{500}{\celsius}$: $v_{\text{mp}} = \SIrange[range-phrase=-]{437}{468}{\meter\per\second}$, $v_{\text{avg}} = \SIrange[range-phrase=-]{474}{508}{\meter\per\second}$, and $v_{\text{rms}} = \SIrange[range-phrase=-]{504}{541}{\meter\per\second}$ \cite{Metcalf1999.LCT}.}.
Immediately after the atoms exit the oven, they first encounter the \SI{461}{\nm} two-dimensional (2D) collimator stage which applies transverse optical molasses \cite{Lett1989.JOSAB.6.2084, Joffe1993.JOSAB.10.2257} to increase to reduce the divergence of the atomic beam although there is no effect on their longitudinal velocity.
Instead of using circular beams, the 2D collimator beams are elliptical to increase the intensity along the atomic beam.

Since the atoms are still moving too quickly to be captured by a magneto-optical trap (MOT)

The atoms next enter the Zeeman slower where a spatially-varying axial magnetic field keeps the atoms in resonance as they slow so that they continuiously scatter photons \cite{Phillips1982.PRL.48.596}.
Our Zeeman slower is a ``spin-flip''-type where the axial magnetic field crosses zero between the ends of the Zeeman slower.
...
Once the atoms exit the Zeeman slower and enter the main chamber, a standard six-beam \SI{461}{\nm} ``blue'' magneto-optical trap (MOT) captures and cools the atoms to about $\SIrange[range-phrase=-]{1}{2}{\milli\kelvin}$.
...
The magnetic trap is also essential when loading multiple isotopes where we can store one isotope in the magnetic trap, move the \SI{461}{\nm} laser to trap another isotope.
...
An overview of our vacuum system is shown in \cref{fig:vacuum_system} and follows a simple and straightforward setup for producing cold and ultracold strontium gases.
Similar designs are described in \cite{Boyd2007.PhD, Stellmer2013.PhD, Barker2016.PhD}.
\begin{figure}[htbp]
	\centering
...
This section of the vacuum chamber is where strontium is converted from its solid metallic state to a gaseous form.
Due to strontium's extremely low vapor pressure, an oven is used to increase the vapor pressure enough to form an atomic beam.
