...
**** Quick overview to give context. Add relevant strontium level diagram figure. ****
...
\begin{table}[htbp]
	\caption[]{
		...
		Calculated from the weighted averages of values in \cref{tab:blue_isotope_shift_detailed,tab:hyperfine_coefficients_5s5p1P1}.}
	\centering
	\begin{tabular}{@{}cccc@{}}
		\toprule
...
Some experiments (e.g., \cite{Barker2016.PhD}) implement a ``cold blue MOT'' stage where detuning, intensity, and quadrupole gradient are ramped during the repumping process to obtain colder temperatures for transfer to the red MOT but we found that our temperatures are sufficient for decent transfer.
...
Of the $\nSLJ{5s5p}{3}{P}{2}$ states, the low-field seeking $m_J = 1, 2$ states can become trapped in the quadrupole magnetic field of the blue MOT \cite{Nagel2003.PRA.67.011401}.
...
One of the key advantages of this transition is that the upper state is $\SLJ{3}{P}{2}$ meaning it only requires a single laser to clear out the magnetic trap since the $\nSLJ{5p^2}{3}{P}{2}$ can only decay to either the $\nSLJ{5s5p}{3}{P}{1}$ or $\nSLJ{5s5p}{3}{P}{2}$ states. 
...
\begin{figure}[htbp]
	...
	\caption[]{\label{fig:repumper_spectra}
		Repumper spectra. *** FINISH CAPTION!! ***}
\end{figure}
...
It is during the switching time that the repump light is applied so all the isotopes in the metastable reservoir are returned to the $\nSLJ{5s^2}{1}{S}{0}$ ready for the red MOT to capture and cool. 
After laser cooling on the \SI{689}{\nm} transition, our sample temperatures are typically $\sim\SIrange[range-phrase=-]{1}{2}{\micro\kelvin}$, which is sufficient for loading into an optical dipole trap (ODT).
...
Since the amount of shift a state experiences is dependent on the intensity, an intensity gradient leads to spatially-varying force such that, when appropriately designed, will attract atoms to the area of highest intensity (or lowest intensity for the case of blue-detuned trapping beams). 
...
While the fermionic red MOT traps atoms in an ellipsoidal volume around the quadrupole zero, the atoms in a bosonic red MOT tend to form a thin shell below the quadrupole center \cite{Stellmer2013.PRA.87.013611, Stellmer2014.arXiv.1307.0601}. 
...
Many experiments have found success by using a high aspect ratio ``sheet''-like trapping beam with a tight waist along gravity and a looser transverse waist \cite{Stellmer2013.PRA.87.013611, Stellmer2013.PhD, Barker2016.PhD, Oppong2015.MS, Campbell2017.PhD}. 
This geometry matches the bosonic isotopes particularly well. 
