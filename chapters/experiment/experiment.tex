...
Being the third ultracold strontium experiment at Rice, we were able to leverage lessons from previous experiments, colloquially referred to as ``Neutral'' and ``Plasma'', to build quite a capable and flexible system\footnote{Collectively, the three experiments work with atomic strontium in all forms: from the ground state (Neutral) to the ions (Plasma), and everything in between (Rydberg).}.
...
In particular, we have in-vacuum electric field plates which we can use to both cancel stray electric fields as well as applying field ionization ramps and direct the resulting electrons (or ions) towards the microchannel plate (MCP) detector.
...
\begin{description}
	...
	\item[Cooling and trapping on the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{3}{P}{1}$ transition] \hfill \\
		Further cooling is achieved by a MOT driving the narrow ${\Gamma}/{2 \pi} = \SI{7.5}{\kHz}$ transition at $\SI{689}{\nm}$.
...
\begin{figure}[htbp]
	...
	\caption[]{\label{fig:sr_level_diagram_simple}
...
At room temperature, strontium is a solid metal with negligible vapor pressure and thus requires an oven to sublimate strontium in to a gaseous form. 
...
\begin{table}[htbp]
	\caption[]{
		...
		Isotope shifts of the $\SI{461}{\nm}$ $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ transition, relative to \Sr{88}, and the $g$-factors.
...
Blue MOTs typically produces samples of atoms around $\SIrange[range-phrase=-]{1}{2}{\milli\kelvin}$.
Some experiments (e.g., \cite{Barker2016.PhD}) implement a ``cold blue MOT'' stage where detuning, intensity, and quadrupole gradient are ramped to obtain colder temperatures for increased transfer efficiency to the red MOT but we found that our temperatures are sufficient for decent transfer.
...
\begin{figure}[htbp]
	...
	\caption[]{\label{fig:old_blue_MOT}
		A picture of our \Sr{88} blue MOT taken on 2014/08/29 through one of the $\SI{2.75}{\inch}$ viewport for the imaging beam. 
		The atom source and Zeeman slower are located off to the right.
		With the electric field plates are separated by about \SI{1}{\inch}, this blue MOT is likely has a diameter of close to $\SI{0.5}{\inch}$.}
\end{figure}
...
This decay path, initially seen as a loss from the blue MOT, ends up being a powerful tool for accumulating atoms in metastable reservoir at roughly the same temperature as the blue MOT and was an essential tool in overcoming the $\SI{0.56(2)}{\percent}$ natural abundance of \Sr{84} to produce the first strontium BECs \cite{Stellmer2009.PRL.103.200401, Martinez2009.PRL.103.200402}.
...
\begin{figure}[htbp]
	...
	\caption[]{\label{fig:repumper_spectra}
		Representative number of \Sr{84} and \Sr{87} atoms returned to the $\nSLJ{5s^2}{1}{S}{0}$ ground state from the magnetic trap varying the $\SI{481}{\nm}$ repumper laser frequency.
		The labeled arrows indicate the expected relative line positions of the various $\nSLJf{5s5p}{3}{P}{2}{F}$ states in \Sr{87} calculated using only the hyperfine $A$ and $B$ constants of the $\nSLJ{5s5p}{3}{P}{2}$ state in \cite{Heider1977.PRA.16.1371}.}
\end{figure}
As seen in \cref{fig:repumper_spectra}, the $A$ and $B$ hyperfine constants of the $\nSLJ{5s5p}{3}{P}{2}$ state captures the energy splittings between the various $\nSLJf{5s5p}{3}{P}{2}{F}$ states quite well.
When multiple isotopes are loaded in to the magnetic trap, it will be necessary to apply the appropriate frequency shifts to efficiently repump laser so that all isotopes are recovered. 
...
Keeping a repumper on can result in initially higher blue MOT loading rates by preventing accumulation of atoms in the metastable reservoir which can drastically reduce cycle times. 
This approach may be of interest for experiments looking to have fast cycle times such as in strontium optical tweezer experiments working with \Sr{88} \cite{Cooper2018.PRX.8.041055, Norcia2018.PRX.8.041054, Hanley2018.PhD, Jackson2019.arXiv.1904.03233}.
An interesting idea is mentioned in \cite{Bowden2019.arXiv.1907.13429} where atoms in the magnetic trap are laser cooled using the $\nSLJ{5s5p}{3}{P}{2} \rightarrow \nSLJ{5s4d}{3}{D}{3}$ transition at $\SI{2.92}{\um}$.
...
With $\Gamma/2\pi = \SI{7.5}{\kHz}$, the Doppler limit $T_{D} = \SI{180}{\nano\kelvin}$ is comparable to the recoil limit of $T_{r} = \SI{229}{\nano\kelvin}$.
\begin{table}[htbp]
	\caption[]{
		\label{tab:red_isotope_shift}
		Isotope shifts of the $\SI{689}{\nm}$ $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{3}{P}{1}$ transition, relative to \Sr{88}, and the $g$-factors.
...
While the bosonic isotopes of strontium behave like an ideal $J=0 \rightarrow J=1$ MOT as described in texts (e.g., \cite{Metcalf1999.LCT, Foot2005.Atomic}) and are relatively straightforward to produces samples below $\SI{2}{\micro\kelvin}$, the hyperfine structure of \Sr{87} poses a challenge.
Since the ground state has (essentially) no magnetic moment, the position-dependent Zeeman shift is only experienced by the $\SLJ{3}{P}{1}$ state with an $m_{J}$-dependent magnitude.
This results in the both a restoring force for some positions and transitions and an anti-trapping force for other transitions and positions.
...
This ability compliments the metastable reservoir exceedingly well since, when switching to the red MOT from the blue MOT, the magnetic field gradient is lowered to $\SIrange[range-phrase=-]{1}{10}{\gauss\per\cm}$ and, as a result, releases the atoms from the magnetic trap. 
It is during the switching time that the repump light is applied so all the isotopes in the metastable reservoir are returned to the $\nSLJ{5s^2}{1}{S}{0}$ ground state, ready for the red MOT to capture and cool. 
...
For a two-state atom with ground state $\ket{g}$ and excited state $\ket{e}$ separated by energy $\hbar \omega_0$ and coupled by a laser at $\omega$, the AC Stark shift can be written as (e.g., see \cite{Grimm1999.arXiv.9902072, Steck.QuantumAtomOptics})
\begin{equation}
...
A quick and easy way to generalize \cref{eq:odt_potential} to a multilevel atom is to treat the multiple levels as individual two-level subsystems and then sum the contributions due to off-resonantly coupling in each two-level subsystem. 
An accurate calculation would require diagonalizing the Hamiltonian which includes all the states in the atom since the AC Stark shift affects both states.
...
\begin{table}[h]
	...
	\end{tabular}
\end{table}
The first strontium BECs were achieved with \Sr{84}, which has a convenient scattering length $a_s = \SI{123}{\bohr}$ \cite{Stein2010.EPJD.57.171}, was easily evaporated to quantum degeneracy in low-aspect ratio traps \cite{Martinez2009.PRL.103.200402, Stellmer2009.PRL.103.200401}.
...
Keeping the beams at this height eliminated the need for rigid supporting structures to raise the entire vacuum system along with the associated need for raised optical platforms for getting beams in to the vacuum chamber. 

To facilitate vertically propagating beams, two holes were cut through the table: a semicircle for the vertical 2D collimator arm and a $\SI{10}{\inch}$ diameter circle under the main chamber for MOT and dipole trap beams.
...
The major disadvantage of our current setup is the long distance between the undertable breadboard and the bottom of the main chamber which is about $\SI{12}{\inch}$\footnote{The thickness of the optical table.}, making it difficult to design, install, and align sensitive vertical beams.
...
Although there are various designs for strontium ovens \cite{Senaratne2015.RSI.86.023105, Schioppo2012.RSI.83.103101}, and even commercial systems, we used a simple design that has shown success on the older strontium experiments in the lab.
The design is simple with a single Watlow FIREROD cartridge heater\footnote{These are neat little devices which come with leads for both the resistive heating element and an integrated thermocouple.} providing the majority of the heating power to the atomic oven.
...
\begin{figure}[htbp]
	...
	\caption[]{
		...
		Neutral's new strontium oven during construction before nozzle thermocouple and heating wire have been attached. 
		The design is very similar to our oven but is mounted on a $\SI{2.75}{\inch}$ flange instead of a $\SI{3.375}{\inch}$ flange as on our experiment.
...
To maximize the limited amount $\SI{461}{\nm}$ laser power, the beams are shaped so to be elliptical to maximize the intensity on the atoms and a single beam is passed through the The long arms of the 2D collimator help prevent strontium buildup on the AR coated viewports.
\begin{figure}[htbp]
	\centering
	\includesvg[keepaspectratio, width=\textwidth, height=\textheight]{experiment/atomic_oven/atomic_beam.svg}
	\caption[]{
		\label{fig:atomic_beam}
		Picture of the fluorescing atomic beam as viewed down the vertical arm of the two-dimensional (2D) collimator.
		A reflection of the atomic beam is visible off the vertical retro mirror.}
\end{figure}

Before the atoms leave this section of the vacuum system and enter the Zeeman slower, they pass through a differential pumping tube\footnote{A cylinder of OFHC copper with a coaxial through hole approximate dimensions of $\diameter = \SI{0.3}{\inch}$ by $L = \SI{2.4650}{\inch}$.} with a conductance $C \approx \SI{0.85}{\liter\per\second}$ which helps maintain the pressure differential between the source and experiment sides of the system. 
...
Our Zeeman slower, detailed in \cite{Camargo2015.MS},  so it will only be briefly covered here. 
Our Zeeman slower uses a spatially-varying axial magnetic field and circularly-polarized light to keep the atoms resonant with a light propagating counter to the direction of the atomic beam so that they continuously scatter photons as they slow \cite{Phillips1982.PRL.48.596}.
...
Our Zeeman slower was custom made, detailed in \cite{Camargo2015.MS}, with a double wall that acts as a water cooling jacket the inner wall separates the water from UHV and the outer wall where the magnetic field coils are wrapped. 
...
The Zeeman slower coil was constructed in multiple layers so that subsequent layers can be designed so that the resulting field was matched the desired profile.
Each layer was attached to the Zeeman slower with thermally-conductive epoxy\footnote{Emerson and Cuming STYCAST 2762 or STYCAST 2762 FT with Catalyst 14, 17, or 17M-1.} and measured before wrapping the next layer of windings.
Due to our relatively long Zeeman slower\footnote{The inner wall of the Zeeman slower is about $\diameter = \SI{0.6940}{\inch}$ by $L = \SI{18.8296}{\inch}$.} ($C \approx \SI{1.4}{\liter\per\second}$), it should provide some additional differential pumping between the source and experiment sides.

\subsection{Experiment-side Vacuum System}

The experiment-side vacuum system can be broken down in to the main chamber where experiments take place and a ``pumping tower'' which provides a high-conductance path to the ion pump.
...
\begin{itemize}
	...
	\item Six (6) $\SI{2.75}{\inch}$ horizontal viewports provide optical access for the various MOT and optical dipole trap beams.
	\item Three (3) $\SI{1.33}{\inch}$ horizontal viewports used for the various $\SI{689}{\nm}$ and $\SI{320}{\nm}$ excitations beams.
	These are oriented perpendicular and parallel to the MCP, allowing us to apply bias magnetic fields along the MCP axis to reduce electrons being deflected by the Lorentz force.
...
Since we do not have a 2D deflection MOT, strontium from the atomic beam will deposit on the viewport overtime\footnote{Neutral has tried using Plasma's pulsed Nd:YAG laser to perform ablation on their coated Zeeman viewport with a moderate level of success.}.
A $\SI{3.375}{\inch}$ gate valve separates the Zeeman slower window from the main chamber and facilitates changing the Zeeman slower entry viewport once it gets coated with strontium.
...
\begin{figure}[htbp]
	...
	\caption[]{
		...
		The trim coil axes are defined relative to the MCP (equivalently, the $\SI{1.33}{\inch}$ viewports for the Rydberg excitation lasers) and gravity with the $X$-axis (blue) oriented towards the MCP, the $Z$-axis (red) perpendicular to the table surface/gravity, and the $Y$-axis (green) perpendicular to the other two axes (following the right-hand rule, of course).}
\end{figure}
...
This allows us to use two different current drivers without needing a controller that has the dynamic range to provide the $\SI{\sim 40}{\ampere}$ to generate the $\SI{\sim 40}{\gauss\per\cm}$ field for the blue MOT while also generating a low-noise $\SI{\sim 1}{\gauss\per\cm}$ gradient for the red MOT.
\begin{figure}[htbp]
...
The blue MOT coils are constructed from square tubular wires\footnote{S{\&}W Wire 125SQ DPG/BARE (NEMA 46-C).} which are hollow inside for cooling water and produce a magnetic field gradient of about $\SI{1.25}{\gauss\per\cm\per\ampere}$ along the tight axis.
The coils are typically run with around $\SIrange[range-phrase=-]{40}{50}{\ampere}$ from a power supply\footnote{TDA-Lambda Genesys 8-90-IS510-U which can supply up to $\SI{8}{\volt}$ and $\SI{90}{\ampere}$.} set for fixed-current output with a power MOSFET\footnote{An IXYS IXFN 340N07.} for quickly turning the current on and off.
...
The master laser is locked relative to the $\nSLJ{5s^2}{1}{S}{0} \rightarrow \nSLJ{5s5p}{1}{P}{1}$ in a homebuilt strontium cell using Doppler-free saturated absorption spectroscopy.
The error signal is obtained by modulating the axial magnetic field around the saturated absorption cell and using circularly-polarized pump and probe beams.
...
Although this laser tunes well and generally remains single-mode when tuning between the various isotopes, we have been very disappointed with output mode quality of the that appears to be a Hermite-Gaussian {TEM\tsub{00}} as seen in \cref{fig:VP-0024_output}.
\begin{figure}[htbp]
...
\end{figure}
Due to this poor output mode, a D-mirror was used to split the beam into two nearly-Gaussian beams, each of which were directed to a slave laser for injection locking. 
...
We also noticed some weird grounding and noise issues with the New Focus laser system which seemed very picky about the particular cables connecting the feedback circuit to the driver\footnote{Similar grounding and noise issues with the New Focus lasers were mentioned by Sara Campbell in Jun Ye's group when I asked them if they had issues with the New Focus lasers. She documents their experiences in \cite{Campbell2017.PhD}.}.
...
The current \SI{481}{\nm} repumper system is shown in \cref{fig:repumper_system} with light is provided by a Toptica DL 100\footnote{The laser is equipped with a {LD-0488-0060-1} diode.} which outputs about $\SI{14}{\mW}$ of power that is shared between the Rydberg, Neutral, and Plasma experiments.
\begin{figure}[htbp]
	\centering
...
Originally, when the lab was working with a single isotope, simply tuning the lock point to maximize repumping efficiency of the isotope of interest was sufficient (typically \Sr{84} since it's the least abundant).
Now that we routinely work with multiple isotopes, a free-space electro-optic modulator (EOM) was introduced to to add sidebands to the \SI{481}{\nm} light in order to address multiple bosonic isotopes and the hyperfine shift of \Sr{87}.
...
\end{description}
We currently do not implement a fiber phase noise cancellation system \cite{Ma1994.OL.19.1777, Rauf2018.RSI.89.033103} so it's possible that the light at the slave lasers is broadened to kilohertz-levels over the $\SI{\approx 25}{\meter}$ fibers delivering master light to the Rydberg experiment but it shouldn't be too difficult to implement in the future when necessary. 
...
The $\SI{689}{\nm}$ slave lasers are relatively easy to build, as detailed in \cite{Ding2016.MS}, and there are currently three slave lasers for injection locking. 
...
Strontium in the ground $\nSLJ{5s^2}{1}{S}{0}$ state have no magnetic moment meaning they cannot magnetically trapped. 
This necessitating the use of optical traps. 
...
Although there are modifications to reduce amplitude noise in the Nufern fiber amplifier as described in \cite{Mazurenko2017.PhD, Mazurenko2019.RSI.90.033101}, we use ours witout any modifications. 
...
The drawback to the entirely fibered ODT system is that it is difficult to efficiently couple more than about $\SI{10}{\W}$ continuously through the a fiber without thermal effects degrading the coupling efficiency.
A workaround to this issue is by only have high-power through the fibers during transfer from the red MOT, to produce the deepest trapping potential, before quickly moving to the evaporation stage where the powers are ramped down. 
...
These fibers are air-gapped and have an endcap on the ends that allows the beam to expand before entering/exiting the fiber, reducing the power density. The adjustable fiber tip position allows for optimization of coupling efficiency into the fiber.
Although the fiber tips are very delicate, we routinely coupled about $\SI{10}{\W}$ of output power from these fibers.
...
\end{figure}
The horizontal trap frequency in the main ODT is increased with a circular ODT beam which intersects the sheet trap at a near vertical angle.
...
We attempted to circularly polarized the vertical ODT so that it would minimize the differential shifts and enable cooling in to the ODT \cite{Ido2003.PRL.91.053001, Stellmer2013.PRA.87.013611} but we have not yet observed any improvement in loading the ODT.
This lack of improvement is likely due to our $\SI{689}{\nm}$ laser system being too broad to take advantage of this.
...
Once the Rydberg atoms are created, they are detected by applying an electric field ramps that ionizes any Rydberg atoms created and directing them towards a micro-channel plate (MCP) detector for counting.
...
\begin{description}
	\item[Koheras Basik BMY10PztSPm, KOH2038] \hfill \\
		...
		The narrowest $\SI{640}{\nm}$ lines observed with this seed are around $\SI{1}{\MHz}$, likely limited by the slow feedback bandwidth of the piezo tuning element.
...
Similar to the setup described in \cite{Mieth2014.OE.22.11182}, OPO-SFG is achieved by replacing the standard OPO crystal with a periodically poled, magnesium-oxide-doped lithium niobate crystal ({MgO:PPLN}) where the first section is poled for OPO and second section is poled for SFG.
...
A workaround was developed where a water-cooling block is attached to the outside of the OPO cavity housing to lower the temperature enough to where the Argos' internal heater is capable stabilizing the temperature (see \cref{fig:argos_opo}).
...
The PZT on one end is used to stabilize the cavity length to the $\SI{689}{\nm}$ laser while the other end provides a tunable offset cavity length which can be used to lock to different FSRs of the transfer cavity.
...
The $\SI{640}{\nm}$ output from the OPO is coupled in to a commercial Toptica second-harmonic generation (SHG) pro system, which doubles the $\SI{640}{\nm}$ to $\SI{320}{\nm}$.
...
The electric field plate system, shown in \cref{fig:electric_field_system-CAD,fig:electric_field_system-real}, follows similar ones described in \cite{Millen2011.PhD, Low2012.JPB.45.113001} using a split-ring electrode geometry with four quadrant electrodes above and four quadrant electrodes below the center of the chamber (i.e., where the atoms are).
\begin{figure}[htbp]
	\centering
...
Some of the previous work with strontium Rydberg experiments was carried out in the Neutral apparatus but since it lacked the ability to detect charged particles, atom loss in absorption images were used instead \cite{DeSalvo2015.PRA.92.031403, DeSalvo2016.PRA.93.022709, Aman2016.PRA.93.043425}.
Our system was designed from the beginning to be able to detect charged particles which we've found to be incredibly sensitive with the ability to detect signals spanning several orders of magnitude \cite{Camargo2018.PRL.120.083401}.
...
The following procedure outlines how electrons are detected but the system can be configured to detection ions instead.
...
This non-linearity is important to characterize to determine when the signal changes from the linear regime to a non-linear regime and requiring the non-linearity to be taken in to account. 
\begin{figure}[htbp]
	\centering
