...
fitM2 = 1; %1 for true, 0 for false

% Decide whether to use weights or not
weightFit = 1; %1 for true, 0 for false
...
rayLength = @(w0) pi*w0^2/lambda;
if fitM2
    waistFunc = @(w0,x0,M,x) w0*1e-4.*sqrt(1 + ((x - x0).*M^2./rayLength(w0*1e-4)).^2); 
else
    waistFunc = @(w0,x0,x)   w0*1e-4.*sqrt(1 + ((x - x0)./rayLength(w0*1e-4)).^2);
end
...
rawData    = textscan(dataFileID,'%f%f%f%f%f%f%f','commentstyle','%'); fclose(dataFileID);
