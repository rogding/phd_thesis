...
\begin{equation}
	...
	\hat{V}\pqty*{\vb{r}, \vb{r}^\prime}	=	\frac{4 \pi \hbar^2}{m} a_{s} \delta\pqty*{\vb{r} - \vb{r}^\prime}
\end{equation}
where $\vb{r}$ and $\vb{r}^\prime$ are the positions of the two particles, $a_{s}$ is the $s$-wave scattering length, and $\delta\pqty*{\vb{r}}$ is the Dirac delta function \cite{Pethick.BEC}.
...
\begin{equation}
	...
	\hat{V}\pqty*{\vb{r},\vb{r}^\prime}	=	\sum_{k} \frac{C_{k} \pqty*{\vb{r},\vb{r}^\prime}}{\abs*{\vb{r} - \vb{r}^\prime}^{k}}
\end{equation}
with interaction strength $C_{k}\pqty*{\vb{r},\vb{r}^\prime}$ and the ``long-range'' coming from the $\hat{V} \sim {1}/{\abs*{\vb{r} - \vb{r}^\prime}^{k}}$ dependence \cite{Pethick.BEC}.
...
\begin{table}[!htbp]
	...
	\begin{tabular}{@{}cccc@{}}
		...
		\midrule
		Binding energy		& $\pqty*{n^*}^{-2}$	& $\SI{11.19}{\meV}$										& $\SI{11.35}{\meV}$									\\
		Coulomb splitting	& $\pqty*{n^*}^{-3}$	& $\SI{148.77}{\GHz}$										& $\SI{151.84}{\GHz}$									\\
		Orbital radius		& $\pqty*{n^*}^{2}$		& $\SI{1823.74}{\bohr}$										& $\SI{1798.74}{\bohr}$									\\
		Radiative lifetime	& $\pqty*{n^*}^{3}$		& $\SI{28.6+-1.1}{\us}$ \cite{Branden2009.JPB.43.015002}	& $\SI{21+-1}{\us}$ \cite{Camargo2016.PRA.93.022702}	\\
		\bottomrule
	\end{tabular}
\end{table}
...
In a dense enough gas, the $\sim \pqty*{n^*}^{2}$ scaling means the Rydberg orbital radius can quickly become comparable to or larger than the interparticle separations.
...
Note that the binding energies of ULRRMs are typically a few to a few-tens of megahertz and scale as $E_{b} \sim {1}/{{n^*}^6}$\footnote{Since $\ev*{\hat{V}\pqty*{\va{R}}} \propto {\abs*{\Psi_{nl}\pqty*{\va{R}}}^2}$, the ${1}/{{n^*}^6}$ scaling can be understood as the electron probability being ``spread out'' over a volume which scale as $V \sim \ev*{r}^{3} \sim {n^*}^6$.}, meaning that ULRRMs can only be reliably created in ultracold, dense samples.
