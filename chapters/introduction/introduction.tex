...
\end{equation}
where $\vb{r}$ is the relative distance between two colliding particles, $g$ is the interaction strength\footnote{The effective interaction for two particles of equal mass is $g = {4 \pi \hbar^2 a_{s}}/{m}$ where $a_{s}$ is the $s$-wave scattering length \cite{Pethick.BEC}.}, and $\delta\pqty*{\vb{r}}$ is the Dirac delta function \cite{Pethick.BEC, Dalibard1998.Collisions}.
...
These can also have an angular dependence as well, which can lead to isotropic and anisotropic interactions that add additional ``richness'' to the system. 
This provides the opportunity to study a wide variety of phenomena including those with applications towards quantum computing \cite{Jaksch2000.PRL.85.2208, DeMille2002.88.067901}, quantum simulation \cite{deLeseleuc2018.PRL.120.113602, Schauss2018.QST.3.023001}, and tests of theory.
...
\begin{description}
	\item[Polar molecules] \hfill \\
		...
		The major disadvantage is that their additional complexity generally makes it very difficult to produce ultracold and quantum degenerate samples and to control their interactions.
	...
	\item[Coupling atoms to a high-finesse cavity] \hfill \\
		...
		These systems are also constrained by the geometry of the experiment which could make it difficult to tune between isotropic and anisotropic interactions.
...
With their hydrogen-like electronic structure, there are well-established methods for both calculating their properties (e.g., see \cite{Sibalic2017.CPC.220.319}) and techniques for producing ultracold and quantum degenerate gases, making them very popular for ultracold gas experiments.
...
As will be discussed below, strontium has some properties that make it a very attractive system for studying Rydberg physics. 
...
(The subscripts in \cref{eq:Rydberg_formula} indicate a common dependency of the values of $\delta$ on $n$, $j$, and $l$.)
Oftentimes, the principal quantum number and quantum defect are combined into an ``effective principal quantum number'' ${n^*} = n-\delta_{n,j,l}$. 
This is the Rydberg formula.
Conceptually, a Rydberg atom can be thought of as being comprised of a core ion and a highly-excited valence electron that spends the majority of its time far away from the core.
...
While the details of the core ion and electrons are important, causing $\delta_{n,j,l}$ to vary from atom-to-atom and from orbital-to-orbital, interactions with the core generally do not have a large effect on the overall properties of the Rydberg atoms (e.g., notice the similarities between the $n = 38$ Rydberg states in rubidium and strontium shown in \cref{tab:Rydberg_n-scaling}).
...
One key aspect to focus on from \cref{tab:Rydberg_n-scaling} is that the size of a Rydberg atom increases very rapidly with $n^*$. 
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[keepaspectratio, width=5.25in, height=\textheight]{introduction/rydberg_molecules/orbital_radius-density_n-scaling.pdf}
	\caption[]{
...
Notice that, even for modest principal quantum numbers and densities, it's easily possible to excite Rydberg atoms with sizes much larger than the interparticle separations (i.e.,  $\ev*{r} \gg {\rho^{-{1}/{3}}}$).

This shift was first explained by Fermi \cite{Fermi1934.NC.11.157}, and later generalized by Omont \cite{Omont1977.JPF.38.1343}, to be the result of low-energy scattering of the nearly free Rydberg electron off nearby neutral atoms, describing the electron-atom interaction with the pseudopotential
\begin{equation}
	\hat{V}\pqty*{\vb{R}}
...
Note that the binding energies of ULRRMs are typically a few to a few-tens of megahertz and scale as $E_{b} \sim {\pqty*{n^*}^{-6}}$\footnote{Since $\ev*{\hat{V}\pqty*{\vb{R}}} \propto {\abs*{\Psi_{nl}\pqty*{\vb{R}}}^2}$, the $\pqty*{n^*}^{-6}$ scaling can be understood as the electron probability being ``spread out'' over a volume which scale as $\sim \ev*{r}^{3} \sim \pqty*{n^*}^6$.}, meaning that ULRRMs can only be created in ultracold, dense samples.
...
\begin{figure}[h]
	...
	\caption{
		...
		Energies and transition probabilities are from \cite{Sansonetti2010.JPCRD.39.033103}. 
		Double arrows indicate driven transitions and dashed arrows indicate decay paths.}
\end{figure}
\footnotetext{Energy level diagram based on ones presented in \cite{Stellmer2013.PhD} and by the Weidem{\"{u}}ller strontium Rydberg experiment at University of Science and Technology of China (USTC) Shanghai Institute for Advanced Studies.}
...
Hopefully this thesis serves as a valuable resource for future studies of strontium Rydberg atoms and molecules.