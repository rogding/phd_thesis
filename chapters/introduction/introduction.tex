...
A very nice derivation of the interaction is presented in \cite{Bendkowsky2010.PhD}\footnote{She references \cite{Balewski2009.Diploma} which has more details of Fermi's approach.} along with some review articles \cite{Shaffer2018.NatComm.9.1965, Fey2019.arXiv.1904.08372, Eiles2019.JPB.52.113001}.

We are particularly interested in the case when, in the low-energy scattering regime, $A_{s} < 0$ because the interaction is attractive and can lead to the formation of bound states for nearby ground state atoms in the located in Rydberg electron wave function.
These objects are known as ``ultra-long-range Rydberg molecules''\footnote{Other objects can also be called ``Rydberg molecules'' (e.g., a molecule electronically excited to a high principal quantum number) but for the purposes of this thesis, a Rydberg molecule refers to the binding of neutral atoms to a Rydberg atom due to scattering.} (ULRRMs) and were theoretically predicted by Greene et al., \cite{Greene2000.PRL.85.2458} in {2000}. 
Instead of the more conventional binding mechanisms (e.g., ionic or covalent), the ground state atoms are bound to the Rydberg atom by the low-energy Rydberg electron-neutral atom scattering. 
...
\Cref{fig:n34_rydberg_molecule_wave_functions_and_spectra} provides an illustrative example of the molecular potential with the resulting radial vibrational wave functions and the spectroscopic signal obtained from experiment. 
\begin{figure}[htbp]
	\centering
...
Note that the binding energies are typically a few to a few-tens of megahertz, it scales as $E_{b} \sim {1}/{{n^\star}^6}$\footnote{The ``volume'' of the electron orbital scales roughly as $V \sim {n^\star}$ meaning.}, meaning the ULRRMs only be reliably created in ultracold and dense samples.
...
A ``trimer'' is one (1) Rydberg atom and two (2) ground-state atoms, a ``tetramer'' is one (1) Rydberg atom and three (3) ground-state atoms, and so on (e.g., see \cite{Gaj2014.NatComm.5.4546}).
...
One aspect that does depend on the details of the interaction occurs (in the context of ULRRMs) is the occurrence of a scattering resonance between the Rydberg electron and nearby perturbing atoms. 
It was observed that the lifetimes of rubidium ULRRMs were significantly shorter than the parent Rydberg state lifetime \cite{Butscher2011.JPB.44.184004} which was attributed to a $p$-wave scattering resonance \cite{Hamilton2002.JPB.35.L199}.
Meanwhile, strontium ULRRMs exhibited similar lifetimes as the parent Rydberg state, indicating an much weaker (or absent) $p$-wave scattering resonance \cite{Camargo2016.PRA.93.022702}.
It should be noted that in very dense gases (i.e., in a BEC), even strontium ULRRMs exhibit reduced lifetimes with the effect being attributed to collisions between the ground-state atom and the Rydberg core ion \cite{Whalen2017.PRA.96.042702}.

\section{****** NOTES TO SELF, REMOVE LATER ******}
...
Mention OFR with Rydberg molecules \cite{Thomas2018.NatComm.9.2238} in ``Conclusion'' chapter?