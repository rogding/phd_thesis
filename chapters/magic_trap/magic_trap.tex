...
\begin{figure}[h]
	...
	\caption{
		...
		Absorption images taken after a $\SI{40}{\ms}$ drop during evaporation in the sheet trap with (top row) and without (middle row) the dimple.
		(Bottom row) Horizontal density profiles with a Gaussian  guide for the eye (green).
		From left to right: $T_{\SI{5}{\s}} \approx \SI{150}{\nano\kelvin}$, $T_{\SI{6}{\s}} \approx \SI{85}{\nano\kelvin}$, $T_{\SI{7}{\s}} \approx \SI{65}{\nano\kelvin}$, $T_{\SI{8}{\s}} \approx \SI{40}{\nano\kelvin}$, and $T_{\SI{11}{\s}} \approx \SI{20}{\nano\kelvin}$.}
\end{figure}
