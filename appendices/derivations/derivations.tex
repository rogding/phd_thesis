...
\section{Optical Dipole Trap Potential}
\label{ap:odt_derivation}

The derivation presented below loosely follows that found in \cite{Steck.QuantumAtomOptics, Ludlow2008.PhD}. For a two-level atom with ground (excited) state $\ket{g}$ ($\ket{e}$) and energy $E_{g} = \hbar \omega_{g}$ ($E_{e} = \hbar \omega_{e}$), the atomic Hamiltonian $H_{A}$ can be written as
\begin{equation}
	H_{A}	=	\hbar
				\begin{pmatrix}
					\omega_{e}	&	0			\\
			...
			=	\hbar
				...
				\end{pmatrix}
\end{equation}
In the dipole approximation, an atom interacting with a field $\va{E} = E_{0} \cos(\omega_{L} t) \vu{\varepsilon} $ leads to the atom-field interaction given by $H_{\text{AF}} = -\va{d} \vdot \va{E} = e\va{r} \vdot \va{E}$.
The matrix elements of $H_{\text{AF}}$ can be written as
\begin{equation}
	H_{AF}	=	\begin{pmatrix}
					\mel{e}{e\va{r} \vdot \va{E}}{e}	&	\mel{e}{e\va{r} \vdot \va{E}}{g}	\\
					\mel{g}{e\va{r} \vdot \va{E}}{e}	&	\mel{g}{e\va{r} \vdot \va{E}}{g}
				\end{pmatrix}
			=	\hbar
				\begin{pmatrix}
					0								&	\Omega \cos(\omega_{L} t)	\\
					\Omega^{*} \cos(\omega_{L} t)	&	0
				\end{pmatrix}
\end{equation}
where the Rabi frequency is defined as $\Omega \equiv {\mel{e}{e\va{r} \vdot \va{E}}{g}}/{\hbar} = -{\mel{e}{\va{d} \vdot \va{E}}{g}}/{\hbar}$. 
...
\begin{equation}
	U	=	\begin{pmatrix}
			...
			\end{pmatrix}
\end{equation}
the total Hamiltonian $H = H_{A} + H_{AF}$ is transformed in to
\begin{equation}
	\widetilde{H}	=		\hbar
							\begin{pmatrix}
								-\Delta													&	\frac{\Omega}{2} \qty(1 + \me^{\iu 2 \omega_L t})	\\
								\frac{\Omega^{*}}{2} \qty(1 + \me^{-\iu 2 \omega_L t})	&	0
							\end{pmatrix}
\end{equation}
where $\Delta \equiv \omega_{L} - \pqty{\omega_{e} - \omega_{g}}$ is the laser detuning from atomic resonance. 
...
\subsection{Calculating the Effect of $\hat{V}_{\text{hf}}$}
This section contains an extended derivation of the hyperfine mixing outlined in \cref{sec:fermion_additional_materials} and was originally presented by Shuhei Yoshida for calculating the hyperfine shifts in the Rydberg states of \Sr{87}.
We begin by constructing the spin ($\va{S}$) and orbital ($\va{L}$) wave functions which couple to produce the $\va{J} = \va{L} + \va{S}$ states. 
The spin wave function ($\ket*{S,m_S}_{S}$) for a two-electron system can be written as
\begin{align}
...
\end{align}
which are the usual singlet and triplet states.
Considering a singly-excited Rydberg state with the ``inner'' electron in the $\ket*{ms}$ state and the ``outer'' electron in the $\ket*{nl}$  state, their combined orbital wave function can be written as
\begin{equation}
	\ket{L, m_{L}}_{L}^{\pm}	=	\frac{1}{\sqrt{2}} \pqty{\ket{ms ; nl} + \ket{nl ; ms}}
\end{equation}
with the superscript indicating the symmetry of state. 
Combining these wave functions gives
\begin{align}
	\ket{\nSLJ{msnl}{1}{L}{J}}	&{}={}	\frac{1}{\sqrt{2}} \ket{L, m_{L}}_{L}^{+} \ket{0,0}_{S}
\end{align}
