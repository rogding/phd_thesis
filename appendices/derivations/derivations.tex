...
Since we (almost) exclusively work with Gaussian laser beams in the lab, I found it instructive to derive the intensity for an astigmatic, elliptical Gaussian beam.
The derivation below mostly follows from the ones presented in \cite{Siegman1986,Arnaud1969.AO.8.1687}.
...
\end{align}
Here, the paraxial approximation $\abs{\partial_{zz}\widetilde{u}} \ll \abs{2 k \partial_{z}\widetilde{u}}, \abs{\partial_{xx}\widetilde{u}}, \abs{\partial_{yy}\widetilde{u}}$ was made which assumes the envelope of the electric field varies ``slowly'' along $z$.
\cref{eq:paraxial_wave_eq} is generally referred to as the ``paraxial wave equation''.

Working in rectangular coordinates, a beam with symmetry along the $x$- and $y$-axis, the paraxial wave equation can be further separated by writing $\widetilde{u}(x,y,z) = \widetilde{u}_{x}(x,z) \widetilde{u}_{y}(y,z)$ where $\widetilde{u}_{x}$ and $\widetilde{u}_{y}$ represent the electric field envelopes along the $x$-axis and $y$-axis, respectively.
...
Interesting discussions about astigmatic Gaussian beams are discussed in \cite{Arnaud1969.AO.8.1687,Kochkina2013.AO.52.6030}.
...
This section contains an extended derivation of the hyperfine mixing presented in \cref{sec:rydberg_hyperfine_mixing}. 
The derivation presented below was originally done by Shuhei Yoshida.

From \cref{sec:rydberg_hyperfine_mixing}, we can approximate the hyperfine interaction of singly-excited Rydberg states (where the inner electron is in the $5s$ state) as
\begin{equation}
...
\subsubsection{Singly Excited Singlet and Triplet Spin States}

Before going to the derivations, it will be helpful to know the singlet and triplet spin wave functions for two electron systems.
Using $\ket{+}$ ($\ket{-}$) for spin-up (spin-down), the singlet and triplet wave functions are  (see e.g., \cite{Sakurai2010})
\begin{align}
	\ket{S=0,m_J=0}		&{}={}	\frac{1}{\sqrt{2}} \left(\ket{+;-} - \ket{-;+}\right)	\\
	\ket{S=1,m_J=1}		&{}={}	\ket{+;+}											\\
	\ket{S=1,m_J=0}		&{}={}	\frac{1}{\sqrt{2}} \left(\ket{+;-} + \ket{-;+}\right)	\\
	\ket{S=1,m_J=-1}	&{}={}	\ket{-;-}
\end{align}
Note that the two electrons are labeled $\ket{s_1;s_2}$.

For a singly excited Rydberg states with the ``inner'' electron in the $\ket{ms}$ state and the Rydberg electron in the $\ket{nl}$ state, the $LS$-coupled $\SLJ{$2S+1$}{L}{$J$}$ Rydberg states can be written as
\begin{align}
	\ket{\nSLJ{msns}{1}{L}{$J$}}	&{}={}	\frac{1}{\sqrt{2}} \left(\ket{ms;nl} + \ket{nl;ms}\right) \ket{S=0,m_S=0}				\notag	\\
									&{}={}	\frac{1}{2} \left(\ket{ms;nl} + \ket{nl;ms}\right) \left(\ket{+;-} - \ket{-;+}\right)	\\
	\ket{\nSLJ{msns}{3}{L}{$J$}}	&{}={}
\end{align}
