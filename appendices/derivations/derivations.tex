...
\begin{equation}
	\abs{\widetilde{u}_{x}\qty(x,z)}^2	=	\sqrt{\frac{2}{\pi}} \frac{1}{w\qty(z)} \exp[-\frac{2 x^2}{w^2(z)}]
\end{equation}

\subsubsection{Complete Solution}

Since $\widetilde{u}_{x}\qty(x,z)$ and $\widetilde{u}_{y}\qty(y,z)$ are both normalized for all $z$, we can set the $x$-axis and $y$-axis beam waists independently.
Letting $z_{x,0}$ and $z_{y,0}$ be the locations of the $x$-axis and $y$-axis waists, the complete solution can be written as
...
\begin{equation}
	\left|\widetilde{u}\left(x,y,z\right)\right|^2
		=	\frac{2}{\pi} \frac{1}{w_{x}(z) w_{y}(z)} \exp\left[-\frac{2x}{w_{x}(z)^2} -\frac{2y}{w_{y}(z)^2}\right]
\end{equation}
where $w_{i}(z) = w(z-z_{i,0})$ for $i \in \{x,y\}$.
